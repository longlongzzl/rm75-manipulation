...
def main(argv=None):
    ...
    parser.add_argument('--search-root',type=Path,action='append',default=[],help='Profile file/directory to scan (repeatable)')
    parser.add_argument('--scan-limit',type=int,default=20000,help='Maximum directory entries inspected')
    parser.add_argument('--doctor',action='store_true',help='Read configuration readiness, without devices or execution')
    args=parser.parse_args(argv)
    if args.list_profiles:
        from rm75_app.workcell.profile_discovery import discover
        result=discover(ROOT,search_roots=args.search_root,explicit=[args.profile] if args.profile else [],max_entries=args.scan_limit)
