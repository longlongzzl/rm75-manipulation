...
from magnetic_snap import MagneticConnection
...
DEFAULT_CUROBO_ROOT = os.environ.get(
    "RM75_MANIPULATION_CUROBO_ROOT",
    str(Path(__file__).resolve().parent / "repro" / "curobo_snapshot"),
)
...
def _install_live_recorder(env: Any, writer: Any | None, record_every: int, profile: dict[str, Any] | None) -> Any:
...
def _load_assembly_state(
    *,
    ...
    log(f"loaded assembly state: {state_path} restored_roles={restored_roles} completed_roles={completed_roles}")
...
def _apply_initial_actor_jitter(
    *,
    ...
    x_min = float(getattr(args, "initial_actor_jitter_safe_x_min", 0.0) or 0.0)
    x_max = float(getattr(args, "initial_actor_jitter_safe_x_max", 0.0) or 0.0)
    y_min = float(getattr(args, "initial_actor_jitter_safe_y_min", 0.0) or 0.0)
    y_max = float(getattr(args, "initial_actor_jitter_safe_y_max", 0.0) or 0.0)
    use_safe_bounds = x_min < x_max and y_min < y_max
    center_x_min = float(getattr(args, "initial_actor_jitter_center_x_min", 0.0) or 0.0)
    center_x_max = float(getattr(args, "initial_actor_jitter_center_x_max", 0.0) or 0.0)
    center_y_min = float(getattr(args, "initial_actor_jitter_center_y_min", 0.0) or 0.0)
    center_y_max = float(getattr(args, "initial_actor_jitter_center_y_max", 0.0) or 0.0)
    use_center_bounds = center_x_min < center_x_max and center_y_min < center_y_max
    ...
    def box_footprint_points(position: np.ndarray, quaternion: np.ndarray, half_size: np.ndarray) -> list[np.ndarray]:
        rotation = quat2mat(quaternion).astype(np.float32)
        points: list[np.ndarray] = []
        for sx in (-1.0, 1.0):
            for sy in (-1.0, 1.0):
                for sz in (-1.0, 1.0):
                    local = np.asarray([sx * half_size[0], sy * half_size[1], sz * half_size[2]], dtype=np.float32)
                    points.append((position + rotation @ local).astype(np.float32))
        return points

    plate_half_size = np.asarray([PLATE_SIZE / 2.0, PLATE_SIZE / 2.0, PLATE_THICKNESS / 2.0], dtype=np.float32)
    actor_baseline_footprints = {
        role: box_footprint_points(*_pose_arrays(_actor_pose(locked[role].actor)), plate_half_size)
        for role in roles
        if role in locked
    }
    fixture_baseline_positions: dict[str, list[tuple[str, np.ndarray]]] = {}
    fixture_baseline_footprints: dict[str, list[tuple[str, list[np.ndarray]]]] = {}
    for fixture in fixtures:
        ...
        fixture_dims = np.asarray(fixture.get("dims", [0.0, 0.0, 0.0]), dtype=np.float32).reshape(-1)
        if fixture_dims.shape[0] >= 3:
            fixture_half_size = fixture_dims[:3] * 0.5
        else:
            fixture_half_size = np.zeros(3, dtype=np.float32)
        for role in roles:
            if name.startswith(f"fixture_{role}_"):
                fixture_baseline_positions.setdefault(role, []).append(
                    (name, fixture_position)
                )
                fixture_baseline_footprints.setdefault(role, []).append(
                    (name, box_footprint_points(fixture_position, fixture_quaternion, fixture_half_size))
                )
    sampled_offsets: dict[str, np.ndarray] = {}
    rejected_counts = {
        "too_close_to_start": 0,
        "too_close_to_target": 0,
        "outside_safe_bounds": 0,
        "outside_center_bounds": 0,
    }

    def role_group_points(role: str, offset: np.ndarray) -> list[np.ndarray]:
        points = [baseline_positions[role] + offset]
        points.extend(position + offset for _, position in fixture_baseline_positions.get(role, []))
        return points

    def role_safe_footprint_points(role: str, offset: np.ndarray) -> list[np.ndarray]:
        points = [point + offset for point in actor_baseline_footprints.get(role, [])]
        for _, footprint in fixture_baseline_footprints.get(role, []):
            points.extend(point + offset for point in footprint)
        return points
    ...
    def is_inside_safe_bounds(role: str, offset: np.ndarray) -> bool:
        if not use_safe_bounds:
            return True
        for point in role_safe_footprint_points(role, offset):
            if not (x_min <= float(point[0]) <= x_max and y_min <= float(point[1]) <= y_max):
                return False
        return True

    def is_inside_center_bounds(role: str, offset: np.ndarray) -> bool:
        if not use_center_bounds:
            return True
        point = baseline_positions[role] + offset
        return (
            center_x_min <= float(point[0]) <= center_x_max
            and center_y_min <= float(point[1]) <= center_y_max
        )

    def is_valid_offset(role: str, offset: np.ndarray) -> bool:
        candidate_position = baseline_positions[role] + offset
        if not is_inside_safe_bounds(role, offset):
            rejected_counts["outside_safe_bounds"] += 1
            return False
        if not is_inside_center_bounds(role, offset):
            rejected_counts["outside_center_bounds"] += 1
            return False
        if min_start_distance > 0.0:
            for other_role, other_offset in sampled_offsets.items():
                for point in role_group_points(role, offset):
                    for other_point in role_group_points(other_role, other_offset):
                        if float(np.linalg.norm(point[:2] - other_point[:2])) < min_start_distance:
                            rejected_counts["too_close_to_start"] += 1
                            return False
        if min_target_distance > 0.0:
            for target_role, target_pose in targets.items():
                ...
                if float(np.linalg.norm(candidate_position[:2] - target_position[:2])) < min_target_distance:
                    rejected_counts["too_close_to_target"] += 1
                    return False
        return True
    ...
    for role in roles:
        ...
        for _ in range(max_attempts):
            ...
            if is_valid_offset(role, offset):
    ...
    report = {
        ...
        "min_start_distance_applies_to": "actor_and_fixture_group_points",
        "safe_bounds": {
            "enabled": use_safe_bounds,
            "x_min": x_min if use_safe_bounds else None,
            "x_max": x_max if use_safe_bounds else None,
            "y_min": y_min if use_safe_bounds else None,
            "y_max": y_max if use_safe_bounds else None,
            "applies_to": "actor_and_fixture_footprint_points",
        },
        "center_bounds": {
            "enabled": use_center_bounds,
            "x_min": center_x_min if use_center_bounds else None,
            "x_max": center_x_max if use_center_bounds else None,
            "y_min": center_y_min if use_center_bounds else None,
            "y_max": center_y_max if use_center_bounds else None,
            "applies_to": "actor_centers",
        },
...
def _apply_target_pose_override(
    targets: dict[str, Any],
    args: argparse.Namespace,
    log: Any,
) -> dict[str, Any]:
    role = str(getattr(args, "override_target_role", "") or "").strip()
    if not role:
        return {}
    if role not in targets:
        log(f"target override skipped: unknown role {role}")
        return {"role": role, "success": False, "reason": "unknown_role"}
    position = np.asarray(
        [
            float(getattr(args, "override_target_x", 0.0)),
            float(getattr(args, "override_target_y", 0.0)),
            float(getattr(args, "override_target_z", 0.0)),
        ],
        dtype=np.float32,
    )
    quaternion_values = np.asarray(
        [
            float(getattr(args, "override_target_qw", 0.0)),
            float(getattr(args, "override_target_qx", 0.0)),
            float(getattr(args, "override_target_qy", 0.0)),
            float(getattr(args, "override_target_qz", 0.0)),
        ],
        dtype=np.float32,
    )
    if float(np.linalg.norm(quaternion_values)) <= 0.0:
        _, quaternion = _pose_arrays(targets[role])
    else:
        quaternion = quaternion_values / max(float(np.linalg.norm(quaternion_values)), 1e-8)
    original = targets[role]
    targets[role] = sapien.Pose(p=position.tolist(), q=quaternion.tolist())
    report = {
        "role": role,
        "success": True,
        "original_target": _pose_to_report(original),
        "override_target": _pose_to_report(targets[role]),
    }
    log(f"target override applied: {role} -> p={position.tolist()} q={quaternion.tolist()}")
    return report
...
def _center_angle_values_for_fast_chain(args: argparse.Namespace) -> list[float]:
    text = str(getattr(args, "fast_chain_center_angle_degs", "") or "").strip()
    if not text:
        return []
    max_abs = abs(float(getattr(args, "fast_chain_center_angle_max_abs_deg", 45.0) or 45.0))
    values: list[float] = []
...
def _candidate_with_center_angle(candidate: Any, angle_deg: float) -> Any:
    angle = float(angle_deg)
    base_angle = float(getattr(candidate, "center_angle_deg", 0.0) or 0.0)
    if abs(angle) <= 1e-6 and abs(base_angle) <= 1e-6:
        return candidate
    yaw = np.deg2rad(angle)
    c = float(np.cos(yaw))
    s = float(np.sin(yaw))
    local_x = float(candidate.local_x)
    local_y = float(candidate.local_y)
    rotated_x = c * local_x - s * local_y
    rotated_y = s * local_x + c * local_y
    label = str(candidate.label)
    if abs(angle) > 1e-6:
        label = f"{label}_center_angle_{angle:+.0f}"
    return SimpleNamespace(
        label=label,
        local_x=float(rotated_x),
        local_y=float(rotated_y),
        thin_bias=float(candidate.thin_bias),
        approach_bias=float(candidate.approach_bias),
        yaw_deg=float(candidate.yaw_deg) + angle,
        pregrasp_distance=float(candidate.pregrasp_distance),
        approach=str(candidate.approach),
        approach_tilt_deg=float(candidate.approach_tilt_deg),
        center_angle_deg=angle,
        base_candidate_label=str(candidate.label),
    )


def _expand_fast_chain_center_angle_candidates(args: argparse.Namespace, candidates: list[Any]) -> list[Any]:
    per_angle = int(getattr(args, "fast_chain_center_angle_candidates_per_angle", 0) or 0)
    angles = _center_angle_values_for_fast_chain(args)
    if per_angle <= 0 or not angles:
        return candidates
    expanded: list[Any] = []
    base = candidates[:per_angle]
    for angle in angles:
        for candidate in base:
            expanded.append(_candidate_with_center_angle(candidate, angle))
    return expanded
...
def _fast_chain_preselect_grasp_candidates(
    *,
    ...
    report: dict[str, Any] = {
        ...
        "pair_probe_all_coupled_tilt": bool(getattr(args, "fast_chain_pair_probe_all_coupled_tilt", True)),
        "grasp_tilt_weight": float(getattr(args, "fast_chain_grasp_tilt_weight", 0.0)),
        "prefer_zero_tilt_pairs": bool(getattr(args, "fast_chain_prefer_zero_tilt_pairs", False)),
        "center_angle_degs": _center_angle_values_for_fast_chain(args),
        "center_angle_candidates_per_angle": int(
            getattr(args, "fast_chain_center_angle_candidates_per_angle", 0) or 0
        ),
    }
    candidate_pool = _expand_fast_chain_center_angle_candidates(args, candidate_pool)
    report["candidate_count_after_center_angle_expansion"] = len(candidate_pool)
    if (
    ...
    try:
        ...
        for item, result in zip(grasp_ok, lift_results):
            ...
            tilt_abs = abs(float(getattr(candidate, "approach_tilt_deg", 0.0)))
            center_angle_abs = abs(float(getattr(candidate, "center_angle_deg", 0.0) or 0.0))
            score = (
                ...
                + float(getattr(args, "fast_chain_grasp_tilt_weight", 0.0)) * tilt_abs
                + float(getattr(args, "fast_chain_center_angle_weight", 0.0)) * center_angle_abs
            )
            ranked.append(
                {
                    ...
                    "grasp_tilt_deg": float(getattr(candidate, "approach_tilt_deg", 0.0)),
                    "center_angle_deg": float(getattr(candidate, "center_angle_deg", 0.0) or 0.0),
                }
            )
        ...
        if not ranked:
            ...
            return candidate_pool, [], report
        if target_actor_pose is not None and bool(getattr(args, "fast_chain_pair_intersection_screening", False)):
            ranked, pair_intersection_report = _select_release_from_pair_intersection(
                planner=planner,
                base_env=base_env,
                locked=locked,
                fixtures=fixtures,
            ...
            report["pair_intersection_screening"] = pair_intersection_report
            if bool(pair_intersection_report.get("selected")):
                report["pair_release_success_count"] = int(pair_intersection_report.get("release_ik_ok_count", 0) or 0)
            if bool(pair_intersection_report.get("enabled")) and pair_intersection_report.get("status") == "Success":
                ranked = list(ranked)
            elif not bool(getattr(args, "fast_chain_allow_fallback", True)):
                report["status"] = "NO_PAIR_INTERSECTION_WINNER"
                report["elapsed_sec"] = time.perf_counter() - started
                return candidate_pool, [], report
        requested_probe_count = max(int(getattr(args, "fast_chain_pair_probe_candidates", 0)), 0)
        has_tilted_ranked = any(
            abs(float(getattr(item["candidate"], "approach_tilt_deg", 0.0))) > 1e-6 for item in ranked
        )
        probe_count = requested_probe_count
        if (
            target_actor_pose is not None
            and requested_probe_count > 0
            and bool(getattr(args, "couple_wall_grasp_release_tilt", False))
            and bool(getattr(args, "fast_chain_pair_probe_all_coupled_tilt", True))
            and has_tilted_ranked
        ):
            probe_count = len(ranked)
            report["pair_release_probe_expanded_for_coupled_tilt"] = True
            report["pair_release_probe_requested_count"] = int(requested_probe_count)
        if target_actor_pose is not None and probe_count > 0:
            ...
            for item in ranked[:probe_count]:
                if isinstance(item.get("release_screen"), dict) and bool(item["release_screen"].get("success")):
                    release_screen = item["release_screen"]
                else:
                    release_screen = _screen_release_for_grasp(
                        ...
                        start_q=item["q_lift"],
                        args=args,
                        matched_tilt_deg=float(getattr(item["candidate"], "approach_tilt_deg", 0.0)),
                    )
            ...
            report["pair_release_probe_effective_count"] = int(probe_count)
        ...
        report["selected"] = [
            {
                ...
                "center_angle_deg": float(item.get("center_angle_deg", 0.0)),
...
def _release_candidate_entries(
    ...
    role: str,
    target_actor_pose: sapien.Pose,
    matched_tilt_deg: float | None = None,
    ...
    for index, (label, release_actor_pose) in enumerate(
        _release_candidates_for_role(args, role, target_actor_pose, matched_tilt_deg=matched_tilt_deg)
    ):
        if index_filter:
...
def _fast_chain_preselect_release_candidates(
    *,
    ...
    entries = _release_candidate_entries(args, role, target_actor_pose, matched_tilt_deg=matched_tilt_deg)
...
def _batch_report_from_result(result: Any, *, joint_distance_to_start: float) -> dict[str, Any]:
    return {
        ...
        "debug": getattr(result, "debug", {}),
        "fast_chain_cached": True,
        "joint_distance_to_start": float(joint_distance_to_start),
    }


def _select_release_from_pair_intersection(
    *,
    planner: RM75CuRoboPlanner,
    base_env: Any,
    locked: dict[str, Any],
    fixtures: list[dict[str, Any]],
    role: str,
    grasp_ranked: list[dict[str, Any]],
    target_actor_pose: sapien.Pose,
    args: argparse.Namespace,
    ...
    report: dict[str, Any] = {
        "enabled": True,
        "grasp_candidate_count_in": len(grasp_ranked),
        "max_grasps": int(getattr(args, "fast_chain_pair_intersection_max_grasps", 0) or 0),
        "max_pairs": int(getattr(args, "fast_chain_pair_intersection_max_pairs", 0) or 0),
    }
    if not grasp_ranked:
        report["status"] = "NO_GRASP_INPUT"
        report["elapsed_sec"] = time.perf_counter() - started
        return grasp_ranked, report
    max_grasps = max(int(getattr(args, "fast_chain_pair_intersection_max_grasps", 0) or 0), 1)
    max_pairs = max(int(getattr(args, "fast_chain_pair_intersection_max_pairs", 0) or 0), 1)
    ik_seeds = max(int(getattr(args, "fast_chain_ik_seeds", min(int(args.ik_seeds), 16))), 1)
    connection_potential = _wall_connection_potential(base_env, role)
    evaluate_all_modes = str(getattr(args, "wall_release_approach_mode", "auto") or "auto").strip().lower() == "both"
    approach_modes = _wall_release_approach_modes(args, role, connection_potential)
    report["connection_potential"] = int(connection_potential)
    report["approach_modes"] = list(approach_modes)
    if bool(getattr(args, "fast_chain_pair_intersection_group_by_center_angle", False)):
        per_angle = max(
            int(getattr(args, "fast_chain_pair_intersection_grasps_per_center_angle", 0) or 0),
            1,
        )
        grouped: dict[float, list[dict[str, Any]]] = {}
        angle_order: list[float] = []
        for item in grasp_ranked:
            angle = round(float(item.get("center_angle_deg", 0.0) or 0.0), 6)
            if angle not in grouped:
                grouped[angle] = []
                angle_order.append(angle)
            if len(grouped[angle]) < per_angle:
                grouped[angle].append(item)
        selected_grasps: list[dict[str, Any]] = []
        for angle in angle_order:
            selected_grasps.extend(grouped.get(angle, []))
            if len(selected_grasps) >= max_grasps:
                break
        selected_grasps = selected_grasps[:max_grasps]
        report["group_by_center_angle"] = True
        report["selected_center_angle_counts"] = {
            str(angle): len(grouped.get(angle, [])) for angle in angle_order
        }
    else:
        selected_grasps = grasp_ranked[:max_grasps]
    pairs: list[dict[str, Any]] = []
    for grasp_item in selected_grasps:
        actor_to_tcp = grasp_item["nominal_actor_to_tcp"]
        for release_mode in approach_modes:
            release_args = _args_for_release_mode(args, role, release_mode)
            entries = _release_candidate_entries(
                release_args,
                role,
                target_actor_pose,
                matched_tilt_deg=float(getattr(grasp_item["candidate"], "approach_tilt_deg", 0.0)),
            )
            for release_index, label, release_actor_pose, _cache in entries:
                target_tcp = release_actor_pose * actor_to_tcp
                preplace, approach_report = _release_preplace_pose_for_candidate(
                    base_env=base_env,
                    role=role,
                ...
                pairs.append(
                    {
                        "grasp": grasp_item,
                        "grasp_tilt_deg": float(getattr(grasp_item["candidate"], "approach_tilt_deg", 0.0)),
                        "release_args": release_args,
                        "release_mode": release_mode,
                        "release_index": int(release_index),
                        "label": str(label),
                        "center_angle_deg": float(grasp_item.get("center_angle_deg", 0.0) or 0.0),
                        "release_actor_pose": release_actor_pose,
                        "target_tcp": target_tcp,
                        "preplace": preplace,
                        "release_approach": approach_report,
                ...
                if len(pairs) >= max_pairs:
                    break
            if len(pairs) >= max_pairs or (pairs and not evaluate_all_modes):
                break
        if len(pairs) >= max_pairs:
            break
    report["pair_count_in"] = len(pairs)
    if not pairs:
        report["status"] = "NO_RELEASE_PAIRS"
        report["elapsed_sec"] = time.perf_counter() - started
        return grasp_ranked, report
    ...
    pre_results = _batch_ik_to_poses(
        ...
        locked=locked,
        fixtures=fixtures,
        start_qs=[np.asarray(item["grasp"]["q_lift"], dtype=np.float32).reshape(-1)[:7] for item in pairs],
        poses=[item["preplace"] for item in pairs],
        num_seeds=ik_seeds,
        exclude_role=role,
        use_cuda_graph_batch=bool(getattr(args, "use_cuda_graph_batch_ik", False)),
        cuda_graph_batch_size=int(getattr(args, "cuda_graph_batch_ik_max_batch", 0) or 0) or None,
    ...
    pre_ok: list[dict[str, Any]] = []
    for pair, result in zip(pairs, pre_results):
        if not bool(getattr(result, "success", False)) or getattr(result, "goal_joint", None) is None:
            continue
        q_preplace = np.asarray(result.goal_joint, dtype=np.float32).reshape(-1)[:7]
        pre_delta = _joint_distance(q_preplace, pair["grasp"]["q_lift"])
        release_preplace_max_joint_delta = float(getattr(pair["release_args"], "release_preplace_max_joint_delta", 0.0))
        if release_preplace_max_joint_delta > 0.0 and pre_delta > release_preplace_max_joint_delta:
            continue
        copied = dict(pair)
        copied["q_preplace"] = q_preplace
        copied["preplace_joint_delta"] = float(pre_delta)
        copied["pre_report"] = _batch_report_from_result(result, joint_distance_to_start=pre_delta)
        pre_ok.append(copied)
    report["preplace_ik_ok_count"] = len(pre_ok)
    if not pre_ok:
        report["status"] = "NO_PREPLACE_IK_WINNER"
        report["elapsed_sec"] = time.perf_counter() - started
        return grasp_ranked, report

    release_exclude_roles_by_pair = [
        _release_collision_exclude_roles(item["release_args"], role)
        for item in pre_ok
    ]
    first_exclude_roles = release_exclude_roles_by_pair[0] if release_exclude_roles_by_pair else set()
    same_exclude_roles = all(set(item) == set(first_exclude_roles) for item in release_exclude_roles_by_pair)
    if same_exclude_roles:
        place_results = _batch_ik_to_poses(
            planner=planner,
            base_env=base_env,
            locked=locked,
            ...
            num_seeds=ik_seeds,
            exclude_role=None,
            exclude_roles=first_exclude_roles,
            use_cuda_graph_batch=bool(getattr(args, "use_cuda_graph_batch_ik", False)),
            cuda_graph_batch_size=int(getattr(args, "cuda_graph_batch_ik_max_batch", 0) or 0) or None,
            cuda_graph_fixed_batch_size=int(getattr(args, "cuda_graph_batch_ik_fixed_batch_size", 0) or 0) or None,
        )
        ...
        place_results = []
        for item, exclude_roles in zip(pre_ok, release_exclude_roles_by_pair):
            place_results.extend(
                _batch_ik_to_poses(
                    planner=planner,
                    base_env=base_env,
                    locked=locked,
                    fixtures=fixtures,
                    start_qs=[item["q_preplace"]],
                    poses=[item["target_tcp"]],
                    num_seeds=ik_seeds,
                    exclude_role=None,
                    exclude_roles=exclude_roles,
                    use_cuda_graph_batch=bool(getattr(args, "use_cuda_graph_batch_ik", False)),
    ...
    for item, result in zip(pre_ok, place_results):
        if not bool(getattr(result, "success", False)) or getattr(result, "goal_joint", None) is None:
            continue
        q_place = np.asarray(result.goal_joint, dtype=np.float32).reshape(-1)[:7]
        place_delta = _joint_distance(q_place, item["q_preplace"])
        release_max_joint_delta = float(getattr(item["release_args"], "release_max_joint_delta", 0.0))
        if release_max_joint_delta > 0.0 and place_delta > release_max_joint_delta:
            continue
        predicted_report = _predicted_actor_pose_error_from_tcp_q(
            planner=planner,
            base_env=base_env,
            q=q_place,
            actor_to_tcp=item["grasp"]["nominal_actor_to_tcp"],
            target_actor_pose=target_actor_pose,
        )
        if _release_prediction_gate_failed(predicted_report, item["release_args"]):
            continue
        connection_geometry = _release_connection_geometry_report(
            base_env=base_env,
            role=role,
            release_actor_pose=item["release_actor_pose"],
            args=item["release_args"],
        )
        release_score = _release_candidate_score(
            label=item["label"],
            pose_error=item["pose_error"],
            preplace_joint_delta=float(item["preplace_joint_delta"]),
            place_joint_delta=float(place_delta),
            preplace_joint_weight=float(getattr(item["release_args"], "release_score_preplace_joint_weight", 0.005)),
            place_joint_weight=float(getattr(item["release_args"], "release_score_place_joint_weight", 0.0015)),
        )
        place_report = _batch_report_from_result(result, joint_distance_to_start=place_delta)
        release_score += _release_ik_error_penalty(
            preplace_report=item["pre_report"],
            place_report=place_report,
            args=item["release_args"],
        )
        total_score = (
            float(item["grasp"].get("score", 0.0))
            + float(getattr(args, "fast_chain_release_probe_weight", 0.6)) * float(release_score)
        )
        cached_screen = {
            "success": True,
            "selected_index": int(item["release_index"]),
            "selected": str(item["label"]),
            "selected_score": float(release_score),
            "selected_release_mode": item["release_mode"],
            "release_approach": item["release_approach"],
            "selected_connection_geometry": connection_geometry,
            "selected_predicted_actor_pose_after_place": predicted_report,
            "release_actor_pose": item["release_actor_pose"],
            "target_tcp_pose": item["target_tcp"],
            "grasp_tilt_deg": float(item.get("grasp_tilt_deg", 0.0)),
            "center_angle_deg": float(item.get("center_angle_deg", 0.0)),
            "q_preplace": item["q_preplace"],
            "q_place": q_place,
            "reports": [
                {
                    "index": int(item["release_index"]),
                    "label": str(item["label"]),
                    "success": True,
                    "fast_chain_pass": "pair_intersection",
                    "grasp_tilt_deg": float(item.get("grasp_tilt_deg", 0.0)),
                    "center_angle_deg": float(item.get("center_angle_deg", 0.0)),
                    "release_mode": item["release_mode"],
                    "release_approach": item["release_approach"],
                    "preplace_ik": item["pre_report"],
                    "place_ik": place_report,
                    "preplace_joint_delta_from_lift": float(item["preplace_joint_delta"]),
                    "place_joint_delta_from_preplace": float(place_delta),
                    "predicted_actor_pose_after_place": predicted_report,
                    "connection_geometry": connection_geometry,
                    "release_score": float(release_score),
                }
            ],
            "fast_chain_pair_intersection": True,
        }
        winner = dict(item["grasp"])
        winner["release_screen"] = cached_screen
        winner["release_probe_score"] = float(release_score)
        winner["score"] = float(total_score)
        winner["pair_intersection_release_index"] = int(item["release_index"])
        winner["pair_intersection_release_label"] = str(item["label"])
        winner["pair_intersection_grasp_tilt_deg"] = float(item.get("grasp_tilt_deg", 0.0))
        winner["pair_intersection_center_angle_deg"] = float(item.get("center_angle_deg", 0.0))
        winner["pair_intersection_connection_rank"] = tuple(
            connection_geometry.get("rank", (99, 99, float("inf"), float("inf"), float("inf")))
        )
        winners.append(winner)
    report["release_ik_ok_count"] = len(winners)
    if not winners:
        report["status"] = "NO_PAIR_INTERSECTION_WINNER"
        report["elapsed_sec"] = time.perf_counter() - started
        return grasp_ranked, report
    if bool(getattr(args, "fast_chain_center_angle_zero_first_gate", False)):
        zero_winners = [
            item for item in winners
            if abs(float(item.get("pair_intersection_center_angle_deg", 0.0))) <= 1e-6
        ]
        report["zero_first_gate"] = True
        report["zero_first_gate_zero_winner_count"] = len(zero_winners)
        report["zero_first_gate_nonzero_winner_count"] = len(winners) - len(zero_winners)
        if zero_winners:
            winners = zero_winners
    winners.sort(
        key=lambda item: (
            tuple(item.get("pair_intersection_connection_rank", (99, 99, float("inf"), float("inf"), float("inf")))),
            (
                abs(float(item.get("pair_intersection_grasp_tilt_deg", 0.0)))
                if bool(getattr(args, "fast_chain_prefer_zero_tilt_pairs", False))
                else 0.0
            ),
            (
                abs(float(item.get("pair_intersection_center_angle_deg", 0.0)))
                if bool(getattr(args, "fast_chain_prefer_zero_center_angle_pairs", False))
                else 0.0
            ),
            float(getattr(args, "fast_chain_center_angle_pair_weight", 0.0))
            * abs(float(item.get("pair_intersection_center_angle_deg", 0.0))),
            float(item.get("release_probe_score", float("inf"))),
            float(item["score"]),
            int(item["index"]),
    ...
    report["status"] = "Success"
    report["selected_count"] = len(winners)
    report["elapsed_sec"] = time.perf_counter() - started
    report["selected"] = [
        {
            "index": int(item["index"]),
            "label": str(item["label"]),
            "score": float(item["score"]),
            "release_index": int(item.get("pair_intersection_release_index", -1)),
            "release_label": str(item.get("pair_intersection_release_label", "")),
            "grasp_tilt_deg": float(item.get("pair_intersection_grasp_tilt_deg", 0.0)),
            "center_angle_deg": float(item.get("pair_intersection_center_angle_deg", 0.0)),
            "release_probe_score": float(item.get("release_probe_score", 0.0)),
        }
        for item in winners[: int(getattr(args, "fast_chain_top_grasp_candidates", 6))]
    ]
    ...
    *,
    ...
    if len(offsets) <= 1 and min_successes <= 1:
        result = _screen_release_for_actor_to_tcp(
            ...
            start_q=start_q,
            args=args,
            matched_tilt_deg=matched_tilt_deg,
        )
...
def _release_candidates_for_role(
    args: argparse.Namespace,
    role: str,
    ...
    if role != "top_lid" or not str(getattr(args, "top_lid_hinge_tilt_degs", "") or "").strip():
        ...
        if (
            ...
            return _fixed_topdown_wall_release_actor_candidates(args, target_pose, matched_tilt_deg=matched_tilt_deg)
        if (
            role in {"right_wall", "left_wall", "back_wall", "front_wall"}
            and profile == "generic"
        ):
...
def _fixed_topdown_wall_release_actor_candidates(
    args: argparse.Namespace,
    target_pose: sapien.Pose,
    matched_tilt_deg: float | None = None,
) -> list[tuple[str, sapien.Pose]]:
    ...
    couple_tilt = bool(getattr(args, "couple_wall_grasp_release_tilt", False)) and matched_tilt_deg is not None
    if couple_tilt:
        samples = samples if abs(float(matched_tilt_deg)) <= 1e-6 else []
    candidates: list[tuple[str, sapien.Pose]] = []
    ...
    if couple_tilt:
        tilt_degs = [float(matched_tilt_deg)] if abs(float(matched_tilt_deg)) > 1e-6 else []
    else:
        tilt_degs = [
            value
...
def _build_one_role(
    *,
        ...
        for use_graph in pass_modes:
            ...
            for candidate_index, candidate in enumerate(candidate_subset):
                ...
                if not ok or q_grasp is None:
                    if not bool(getattr(args, "grasp_motion_plan_on_ik_fail", False)):
                        candidate_report["failed_at"] = "edge_grasp_ik"
                        report["candidate_reports"].append(candidate_report)
                        continue
                    ok_plan, q_grasp_path, grasp_plan_report = _profile_call(
                        role_timing,
                        "edge_grasp_motion_plan_sec",
                        _plan_motion_to_pose,
                        planner=planner,
                        base_env=base_env,
                        locked=locked,
                    ...
                    candidate_report["edge_grasp_motion_plan"] = grasp_plan_report
                    if not ok_plan or q_grasp_path is None:
                        candidate_report["failed_at"] = "edge_grasp_ik"
                        report["candidate_reports"].append(candidate_report)
                        continue
                    q_grasp = np.asarray(q_grasp_path[-1], dtype=np.float32).reshape(-1)[:7]
                    candidate_report["edge_grasp_motion_plan_fallback"] = True
                ...
                if preselected_release_screen is not None:
                    ...
                    release_screen = _profile_call(
                        ...
                        start_q=q_lift,
                        args=args,
                        matched_tilt_deg=float(getattr(candidate, "approach_tilt_deg", 0.0)),
                    )
    ...
    if not report["grasp_quality_after_close"]["success"]:
        if bool(getattr(args, "force_held_actor_lock_after_failed_grasp", False)):
            lock_reference = str(getattr(args, "held_actor_lock_reference", "live") or "live").strip().lower()
            forced_actor_to_tcp = nominal_actor_to_tcp if lock_reference == "nominal" else (_actor_pose(actor).inv() * _tcp_pose(base_env))
            realman_module._enable_held_actor_pose_lock(base_env, actor, forced_actor_to_tcp)
            realman_module._apply_held_actor_pose_lock(base_env)
            held_actor_pose_lock_enabled = True
            report["forced_held_actor_pose_lock_after_failed_grasp"] = {
                "enabled": True,
                "reference": lock_reference,
                "actor_to_tcp": _pose_to_report(forced_actor_to_tcp),
                "reason": "physical_grasp_check_failed",
                "grasp_after_close": report["grasp_after_close"],
            }
        else:
            report["success"] = False
            ...
            if profile is not None:
                _accumulate_profile(profile, role_timing)
            return finalize_report()
    if report["grasp_quality_after_close"]["success"] or held_actor_pose_lock_enabled:
        pass
    else:
        report["success"] = False
        report["failed_at"] = "physical_grasp_check"
        ...
        if profile is not None:
            _accumulate_profile(profile, role_timing)
        return finalize_report()
    if bool(getattr(args, "lock_held_actor_after_grasp", False)) and not held_actor_pose_lock_enabled:
        lock_reference = str(getattr(args, "held_actor_lock_reference", "live") or "live").strip().lower()
    ...
    if not report["grasp_quality_after_lift"]["success"]:
        if bool(getattr(args, "force_held_actor_lock_after_failed_grasp", False)) and held_actor_pose_lock_enabled:
            report["forced_held_actor_pose_lock_after_failed_lift_grasp_check"] = {
                "enabled": True,
                "reason": "physical_grasp_lost_during_lift",
                "grasp_after_lift": report["grasp_after_lift"],
            }
        else:
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    segments: list[dict[str, Any]] = []
    reports: list[dict[str, Any]] = []
    env = _make_env(bool(getattr(args, "record_live", False)))
    ...
    try:
        ...
        target_pose_override = _apply_target_pose_override(
            targets=targets,
            args=args,
            log=log,
        )
        ...
        restore_recorder = _install_live_recorder(env, writer, args.record_every, run_profile)
        if bool(getattr(args, "heal_loaded_state_only", False)):
            ...
            manifest = {
                ...
                "assembly_state": {
                    ...
                    "target_pose_override": target_pose_override,
        ...
        if str(getattr(args, "save_assembly_state", "") or "") and not (
            ...
            if all_requested_roles_succeeded:
                ...
                failure_save_roles = _role_list(str(getattr(args, "save_assembly_on_failure_roles", "") or ""))
                partial_roles = list(dict.fromkeys(["floor", *loaded_state_roles, *completed_roles, *failure_save_roles]))
...
def main() -> None:
    ...
    parser.add_argument("--save-assembly-on-failure-roles", default="")
    ...
    parser.add_argument("--initial-actor-jitter-safe-x-min", type=float, default=0.0)
    parser.add_argument("--initial-actor-jitter-safe-x-max", type=float, default=0.0)
    parser.add_argument("--initial-actor-jitter-safe-y-min", type=float, default=0.0)
    parser.add_argument("--initial-actor-jitter-safe-y-max", type=float, default=0.0)
    parser.add_argument("--initial-actor-jitter-center-x-min", type=float, default=0.0)
    parser.add_argument("--initial-actor-jitter-center-x-max", type=float, default=0.0)
    parser.add_argument("--initial-actor-jitter-center-y-min", type=float, default=0.0)
    parser.add_argument("--initial-actor-jitter-center-y-max", type=float, default=0.0)
    parser.add_argument("--override-target-role", default="")
    parser.add_argument("--override-target-x", type=float, default=0.0)
    parser.add_argument("--override-target-y", type=float, default=0.0)
    parser.add_argument("--override-target-z", type=float, default=0.0)
    parser.add_argument("--override-target-qw", type=float, default=0.0)
    parser.add_argument("--override-target-qx", type=float, default=0.0)
    parser.add_argument("--override-target-qy", type=float, default=0.0)
    parser.add_argument("--override-target-qz", type=float, default=0.0)
    ...
    parser.add_argument("--fast-chain-pair-probe-all-coupled-tilt", action=argparse.BooleanOptionalAction, default=True)
    ...
    parser.add_argument("--fast-chain-grasp-tilt-weight", type=float, default=0.0)
    parser.add_argument("--fast-chain-prefer-zero-tilt-pairs", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--fast-chain-center-angle-degs", default="")
    parser.add_argument("--fast-chain-center-angle-max-abs-deg", type=float, default=45.0)
    parser.add_argument("--fast-chain-center-angle-candidates-per-angle", type=int, default=0)
    parser.add_argument("--fast-chain-center-angle-weight", type=float, default=0.0)
    parser.add_argument("--fast-chain-center-angle-pair-weight", type=float, default=0.0)
    parser.add_argument("--fast-chain-prefer-zero-center-angle-pairs", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--fast-chain-center-angle-zero-first-gate", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--fast-chain-pair-intersection-group-by-center-angle", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--fast-chain-pair-intersection-grasps-per-center-angle", type=int, default=1)
    parser.add_argument("--fast-chain-pair-intersection-screening", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--fast-chain-pair-intersection-max-grasps", type=int, default=4)
    parser.add_argument("--fast-chain-pair-intersection-max-pairs", type=int, default=64)
    ...
    parser.add_argument("--grasp-motion-plan-on-ik-fail", action=argparse.BooleanOptionalAction, default=False)
    ...
    parser.add_argument("--couple-wall-grasp-release-tilt", action=argparse.BooleanOptionalAction, default=False)
    ...
    parser.add_argument("--force-held-actor-lock-after-failed-grasp", action=argparse.BooleanOptionalAction, default=False)
