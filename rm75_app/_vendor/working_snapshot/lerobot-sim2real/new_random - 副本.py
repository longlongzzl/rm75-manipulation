...
# JPEG压缩伪影
# -----------------------------------------------------------
class RandomJPEGCompressionK(AugmentationBase2D):
    """
    JPEG压缩伪影：模拟真实相机的JPEG压缩
    通过质量参数控制压缩程度
    """
    def __init__(self, quality_range=(60, 95), p=0.5, same_on_batch=False, keepdim=True):
        super().__init__(p=p, same_on_batch=same_on_batch, keepdim=keepdim)
    ...
    def generate_parameters(self, batch_shape):
        ...
        dev = self.device
        return dict(
            quality=torch.empty(B, device=dev).uniform_(*self.quality_range)
        )
    ...
    def apply_transform(self, x, p, flags=None, transform=None):
        if x.dtype == torch.uint8:
            ...
            x_uint8 = (x.clamp(0, 1) * 255).byte()
        
        p = _to_same_device(p, x.device)
        B, C, H, W = x_uint8.shape
        
        results = []
        for b in range(B):
            quality = int(p["quality"][b].item())
            # 转换为numpy进行JPEG压缩
            img_np = x_uint8[b].permute(1, 2, 0).cpu().numpy()
            if C == 1:
            ...
            img_np_compressed = np.array(img_compressed)
            
            if len(img_np_compressed.shape) == 2:
                img_np_compressed = img_np_compressed[:, :, None]
            
            # 转回tensor
            img_tensor = torch.from_numpy(img_np_compressed).permute(2, 0, 1).float() / 255.0
            if img_tensor.shape[0] != C:
                # 如果是灰度图转RGB，需要复制通道
                img_tensor = img_tensor.repeat(C, 1, 1)
...
# 暗角（Vignetting）
# -----------------------------------------------------------
class RandomVignettingK(AugmentationBase2D):
    """
    暗角效果：模拟镜头边缘变暗
    使用更强的暗角效果，确保可见
    """
    def __init__(self, strength_range=(0.2, 0.7), p=0.3, same_on_batch=False, keepdim=True):
        super().__init__(p=p, same_on_batch=same_on_batch, keepdim=keepdim)
    ...
    def generate_parameters(self, batch_shape):
        ...
        dev = self.device
        return dict(
            strength=torch.empty(B, device=dev).uniform_(*self.strength_range)
        )
    ...
    def apply_transform(self, x, p, flags=None, transform=None):
        ...
        if self._grid is None or self._grid.shape[-2:] != (H, W):
            ...
            # 归一化距离 [0, 1]，使用最大可能距离（从中心到角落）
            dist = torch.sqrt((xx - center_x) ** 2 + (yy - center_y) ** 2)
            max_dist = torch.sqrt(torch.tensor(center_x ** 2 + center_y ** 2, device=x.device, dtype=torch.float32))
            normalized_dist = (dist / (max_dist + 1e-8)).unsqueeze(0)  # (1, H, W)
            self._grid = normalized_dist
        
        grid = self._grid.to(x.device)  # (1, H, W)
        strength = p["strength"].view(B, 1, 1, 1)  # (B, 1, 1, 1)
        
        # 使用更强的暗角公式：使用更高次幂使边缘更暗
        # 使用 grid^3 来增强边缘暗角效果，使边缘更暗
        # 值从1（中心）到(1-strength)（边缘）
        # 使用更激进的公式：vignette = 1 - strength * grid^power
        # power越大，边缘越暗，过渡越陡
        power = 3.0  # 使用3次幂，使边缘更暗
        vignette = 1.0 - strength * (grid ** power)
        vignette = vignette.clamp(0, 1)
        
        # 可选：使用更简单的线性公式（效果更明显但过渡不够平滑）
        # vignette = (1.0 - strength * grid).clamp(0, 1)
        
        return (x * vignette).clamp_(0, 1)


# -----------------------------------------------------------
# 径向畸变（Radial Distortion）
# -----------------------------------------------------------
class RandomRadialDistortionK(AugmentationBase2D):
    """
    径向畸变：模拟镜头畸变（桶形或枕形）
    k1, k2: 畸变系数，负值为桶形，正值为枕形
    """
    def __init__(self, k1_range=(-0.15, 0.15), k2_range=(-0.05, 0.05), p=0.4, same_on_batch=False, keepdim=True):
        super().__init__(p=p, same_on_batch=same_on_batch, keepdim=keepdim)
    ...
    def generate_parameters(self, batch_shape):
        ...
        dev = self.device
        return dict(
            k1=torch.empty(B, device=dev).uniform_(*self.k1_range),
            k2=torch.empty(B, device=dev).uniform_(*self.k2_range)
        )
    ...
    def apply_transform(self, x, p, flags=None, transform=None):
        ...
        for b in range(B):
            ...
            r_distorted = r * (1 + k1 * r2 + k2 * r2 ** 2)
            
            # 计算畸变后的坐标
            scale = r_distorted / (r + 1e-8)
            xx_distorted = xx * scale
            yy_distorted = yy * scale
            
            # 归一化回 [0, 1] 用于grid_sample
            xx_norm = (xx_distorted + 1) / 2 * (W - 1)
            yy_norm = (yy_distorted + 1) / 2 * (H - 1)
            
            # 创建采样网格 (B, H, W, 2)
            grid = torch.stack([xx_norm, yy_norm], dim=-1).unsqueeze(0)  # (1, H, W, 2)
            grid = grid * 2.0 / torch.tensor([W - 1, H - 1], device=x.device) - 1.0  # 归一化到[-1, 1]
            
            # 使用grid_sample进行采样
            x_b = x[b:b+1]  # (1, C, H, W)
            x_distorted = F.grid_sample(
                x_b, grid, mode='bilinear', padding_mode='border', align_corners=True
            )
            results.append(x_distorted.squeeze(0))
        
        return torch.stack(results, dim=0).clamp_(0, 1)


# -----------------------------------------------------------
# 锐化/去锐化（Sharpening/Unsharpening）
# -----------------------------------------------------------
class RandomSharpeningK(AugmentationBase2D):
    """
    锐化/去锐化：模拟ISP处理差异
    strength > 0: 锐化
    strength < 0: 去锐化（模糊）
    """
    def __init__(self, strength_range=(-0.5, 1.0), p=0.4, same_on_batch=False, keepdim=True):
        super().__init__(p=p, same_on_batch=same_on_batch, keepdim=keepdim)
        self.strength_range = strength_range
        # 锐化核
        ...
        self.register_buffer("_sharpen_kernel", sharpen_kernel, persistent=False)
        
        # 高斯模糊核（用于去锐化）
        try:
            from kornia.filters import GaussianBlur2d
            self.gaussian_blur = GaussianBlur2d(kernel_size=(5, 5), sigma=(1.0, 1.0))
        except:
    ...
    def apply_transform(self, x, p, flags=None, transform=None):
        ...
        results = []
        for b in range(B):
            strength = p["strength"][b].item()
            x_b = x[b:b+1]  # (1, C, H, W)
            
            if strength > 0:
                # 锐化
                kernel = self._sharpen_kernel.to(x.device)
                # 对每个通道应用锐化
                sharpened = []
                for c in range(C):
                    channel = x_b[:, c:c+1, :, :]
                ...
                x_processed = torch.cat(sharpened, dim=1)
                # 混合原图和锐化图
                x_b = x_b + strength * (x_processed - x_b)
            else:
                # 去锐化（模糊）
                if self.gaussian_blur is not None:
                    x_blurred = self.gaussian_blur(x_b)
                else:
                    # 简单的平均模糊作为fallback
                    kernel = torch.ones(1, 1, 5, 5, device=x.device) / 25.0
                    x_blurred = F.conv2d(x_b, kernel.repeat(C, 1, 1, 1), padding=2, groups=C)
                # 混合原图和模糊图
                x_b = x_b + abs(strength) * (x_blurred - x_b)
            
            results.append(x_b.squeeze(0))
        
        return torch.stack(results, dim=0).clamp_(0, 1)


# -----------------------------------------------------------
# 局部高光bloom/过曝剪切（Local Highlight Bloom/Overexposure）
# -----------------------------------------------------------
class RandomHighlightBloomK(AugmentationBase2D):
    """
    局部高光bloom和过曝剪切：模拟强光区域的bloom效果和过曝
    """
    def __init__(self, bloom_strength_range=(0.1, 0.4), threshold_range=(0.85, 0.98), 
                 p=0.3, same_on_batch=False, keepdim=True):
        super().__init__(p=p, same_on_batch=same_on_batch, keepdim=keepdim)
        self.bloom_strength_range = bloom_strength_range
        self.threshold_range = threshold_range
        try:
            from kornia.filters import GaussianBlur2d
            self.gaussian_blur = GaussianBlur2d(kernel_size=(15, 15), sigma=(3.0, 3.0))
        except:
            self.gaussian_blur = None
    ...
    def generate_parameters(self, batch_shape):
        ...
        dev = self.device
        return dict(
            bloom_strength=torch.empty(B, device=dev).uniform_(*self.bloom_strength_range),
            threshold=torch.empty(B, device=dev).uniform_(*self.threshold_range)
        )
    ...
    def apply_transform(self, x, p, flags=None, transform=None):
        ...
        results = []
        for b in range(B):
            bloom_strength = p["bloom_strength"][b].item()
            threshold = p["threshold"][b].item()
            
            x_b = x[b:b+1]  # (1, C, H, W)
            
            # 找到高光区域（亮度超过阈值）
            luminance = x_b.mean(dim=1, keepdim=True)  # (1, 1, H, W)
            ...
            if self.gaussian_blur is not None:
                ...
                # 简单的平均模糊作为fallback
                kernel = torch.ones(1, 1, 15, 15, device=x.device) / 225.0
                x_blurred = F.conv2d(x_b, kernel.repeat(C, 1, 1, 1), padding=7, groups=C)
            
            # 混合：高光区域使用模糊版本，其他区域保持原样
            x_bloom = x_b + bloom_strength * highlight_mask * (x_blurred - x_b)
            
            # 过曝剪切：超过1.0的值直接剪切到1.0
            x_bloom = x_bloom.clamp(0, 1)
            
            results.append(x_bloom.squeeze(0))
...
class AugPipeline(nn.Module):
    def __init__(self):
        ...
        self.aug = ImageSequential(
            ...
            # make_oneof需要至少一个未注释的模块，如果全部注释则移除make_oneof
            # make_oneof(
            ...
            # ),
            # Sim2Real常用扰动：JPEG压缩、暗角、畸变、锐化、高光bloom
            # RandomJPEGCompressionK(p=1.0, quality_range=(60, 95)),  # JPEG压缩伪影
            # RandomVignettingK(p=1.0, strength_range=(0.3, 0.8)),  # 暗角（增强效果） # 0.04
            # RandomRadialDistortionK(p=0.4, k1_range=(-0.15, 0.15), k2_range=(-0.05, 0.05)),  # 径向畸变
            # RandomSharpeningK(p=1.0, strength_range=(-0.5, 1.0)),  # 锐化/去锐化
             RandomHighlightBloomK(p=0.3, bloom_strength_range=(0.1, 0.4), threshold_range=(0.85, 0.98)),  # 局部高光bloom/过曝
        )
