...
class Agent(nn.Module):
    def __init__(self, method, sigma, envs, sample_obs, net_type, choice, tcl_choice, device,z_detach, private_dim, search_entropy, act_type, critic_use_pi=0):
        ...
        # 将 state_proj 改为预测器，从图像表示 z_img 预测私有状态信息 private_obs
        # 输入：z_img (temp_out_dim=64维) -> 输出：private_obs (private_dim维)
        self.state_proj = nn.Sequential(
            layer_init(nn.Linear(temp_out_dim, 256)), nn.ReLU(inplace=True),
            layer_init(nn.Linear(256, private_dim)),
        )
    ...
    def prediction_loss(self, pred, target):
        """
        预测损失函数：最小化预测值与目标值之间的L2损失（与DEXTRAH保持一致）
        
        L2损失计算每个样本的L2范数（欧几里得距离），然后取平均。
        相比MSE，L2损失具有更好的物理意义和梯度稳定性。
        
        Args:
            pred: [B, D] 从图像表示 z_img 预测的私有状态信息 private_obs
            target: [B, D] 真实的私有状态信息 private_obs
        ...
        # 使用L2损失（与DEXTRAH项目一致）
        # 对每个样本计算L2范数，然后取平均
        l2_loss = torch.norm(pred - target, p=2, dim=-1).mean()
        ...
        with torch.no_grad():
            ...
            # 计算L2距离（与损失值相同，但保留用于监控）
            l2_dist = torch.norm(pred - target, p=2, dim=-1).mean()
            
            # 计算每个维度的平均误差
            per_dim_error = torch.abs(pred - target).mean(dim=0).mean()
            
            # 计算MSE作为对比指标（不用于训练）
            mse_loss = F.mse_loss(pred, target, reduction='mean')
        
        return l2_loss, {
            "prediction_loss": l2_loss,
            "cosine_similarity": cosine_sim,
            "l2_distance": l2_dist,
            "per_dim_error": per_dim_error,
            "mse_loss": mse_loss,  # 保留MSE作为监控指标，便于对比
        }
...
#                w = w / (w.mean() + 1e-8)  # 归一化到均值=1，避免缩放学习率
    ...
    def get_action_and_value(self, x, action=None, x_next=None, done=None, private_obs=None, org_x = None, alpha = 0.0, adv=None):
        ...
        # 使用 state_proj 从图像表示 z_img 预测私有状态信息 private_obs
        z_state_pred = self.state_proj(z_img)

        # 使用预测损失替代互信息损失
        # z_state_pred 是从 z_img 预测的 private_obs，private_obs 是目标（真实值）
        tcl, plot_info = self.prediction_loss(z_state_pred, private_obs)
