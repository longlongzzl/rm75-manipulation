...
@register_env("SO101GraspCube_two_cameras-v1", max_episode_steps=64)
class SO101GraspCubeEnv_with_two_cameras(BaseDigitalTwinEnv):
    """
    **Task Description:**
    ...
    **Success Conditions:**
    ...
    def __init__(
        ...
        *args,
        ...
        greenscreen_overlay_path_2 = None,
        ...
        **kwargs,
        ...
        self.base_camera_2_settings = base_camera_2_settings
        ...
        # set the camera called "base_camera" to use the greenscreen overlay when rendering
            ...
            assert greenscreen_overlay_path_2 is not None
            #self.rgb_overlay_paths = dict(base_camera=greenscreen_overlay_path)
            self.rgb_overlay_paths = {
                "base_camera": greenscreen_overlay_path,
                "base_camera_2": greenscreen_overlay_path_2,
            }
    ...
    def _default_sensor_configs(self):
        ...
        if self.domain_randomization:
            ...
            camera_fov_noise_2 = self.domain_randomization_config.camera_fov_noise * (
                    2 * self._batched_episode_rng.rand() - 1
            )
        else:
            camera_fov_noise = 0
            camera_fov_noise_2 = 0
        return [
            CameraConfig(
                "base_camera",
                pose=sapien.Pose(),
            ...
            CameraConfig(
                ...
                width=128,
                height=128,
                fov=camera_fov_noise_2 + self.base_camera_2_settings["fov"],
                near=0.01,
                far=100,
                mount=self.camera_mount_2,
            )
        ]
    ...
    def _load_scene(self, options: dict):
        ...
        builder_2.initial_pose = sapien.Pose()
        self.camera_mount_2 = builder_2.build_kinematic("camera_mount_2")
    ...
    def sample_camera_poses(self, n: int):
        ...
        if self.domain_randomization:
            ...
            return randomization.camera.noised_look_at(
                ...
                look_at_noise= 0.0,#self.domain_randomization_config.camera_target_noise,
                view_axis_rot_noise=0.0,#self.domain_randomization_config.camera_view_rot_noise,
                device=self.device,
            )
        else:
    ...
    def sample_camera_poses_2(self, n: int):
        # a custom function to sample random camera poses
        # the way this works is we first sample "eyes", which are the camera positions
        # then we use the noised_look_at function to sample the full camera poses given the sampled eyes
        # and a target position the camera is pointing at
        if self.domain_randomization:
            # in case these haven't been moved to torch tensors on the environment device
            self.base_camera_2_settings["pos"] = common.to_tensor(
                self.base_camera_2_settings["pos"], device=self.device
            )
            self.base_camera_2_settings["target"] = common.to_tensor(
                self.base_camera_2_settings["target"], device=self.device
            )
            self.domain_randomization_config.max_camera_offset = common.to_tensor(
                self.domain_randomization_config.max_camera_offset, device=self.device
            )
            ...
            eyes = randomization.camera.make_camera_rectangular_prism(
                n,
                scale=self.domain_randomization_config.max_camera_offset,
                center=self.base_camera_2_settings["pos"],
                theta=0,
                device=self.device,
            )
            return randomization.camera.noised_look_at(
                eyes,
                target=self.base_camera_2_settings["target"],
                look_at_noise=0.0,  # self.domain_randomization_config.camera_target_noise,
                view_axis_rot_noise=0.0,  # self.domain_randomization_config.camera_view_rot_noise,
                device=self.device,
            )
            ...
            return sapien_utils.look_at(
                eye=self.base_camera_2_settings["pos"],
                target=self.base_camera_2_settings["target"],
            )
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        with torch.device(self.device):
            ...
            self.camera_mount_2.set_pose(self.sample_camera_poses_2(n=b))
    ...
    def _before_control_step(self):
        ...
        if self.domain_randomization:
            ...
            self.camera_mount_2.set_pose(self.sample_camera_poses_2(n=self.num_envs))
    ...
    def evaluate(self):
        ...
        #print(" self.agent.tcp_pos", self.agent.tcp_pos)
