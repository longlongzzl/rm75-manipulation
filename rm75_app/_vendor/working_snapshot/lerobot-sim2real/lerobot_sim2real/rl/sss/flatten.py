...
class Flatten_Multi_RGBDObservationWrapper(gym.ObservationWrapper):
    """
    Flattens the rgbd mode observations into a dictionary with two keys, "rgbd" and "state"
    ...
    def observation(self, observation: Dict):
     ...
     #   assert 1 == 2
        ...
        if self.include_rgb and not self.include_depth:
            #ret["rgb"] = rgb_images
            for rgb_index, rgb in enumerate(rgb_images):
                ret["rgb_"+str(rgb_index)] = rgb
        elif self.include_rgb and self.include_depth:
            assert  3 == 4
            if self.sep_depth:
                ret["rgb"] = rgb_images
