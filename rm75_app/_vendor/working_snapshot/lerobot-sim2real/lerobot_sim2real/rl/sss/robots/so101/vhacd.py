    ...
    def parse_origin(tag):
        ...
        if origin is not None:
            ...
            if "rpy" in origin.attrs:
                ...
                T[:3, :3] = euler2mat(rpy[0],rpy[1],rpy[2], axes="sxyz")
        return T
...
def main():
    ...
    model_dir = Path("D:/Project/Scaling/ManiSkill-main/ManiSkill-main/mani_skill/assets/robots/so101/SO_5DOF_ARM101_8j/")
    flag = False

    cvx_dir = model_dir / "cvx_objs"
    cvx_dir.mkdir(exist_ok=True)

    urdf_path = model_dir / "so101_mod2_with_gripper_collision.urdf"
    new_urdf_path = model_dir / "so101_mod2_with_gripper_collision_cvx.urdf"
