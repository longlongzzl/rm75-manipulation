...
def get_infos(env_id, infos):
    ...
    if env_id == "SO101GraspCube_two_cameras-v1": # Done
        ...
        private_info = torch.cat(
            [infos["obj_pose"], infos["tcp_pose"], infos["obj_pos"], infos["tcp_to_obj_pos"], infos["is_grasped"].unsqueeze(-1)], -1)
...
class ManiSkillVectorEnv(VectorEnv):
    ...
    def step(
        ...
        if dones.any() and self.auto_reset:
            ...
            final_private_state = get_infos(self.env_id,infos)

            obs, infos = self.reset(options=dict(env_idx=env_idx))
            # gymnasium calls it final observation but it really is just o_{t+1} or the true next observation
            infos["final_observation"] = final_obs

            infos["final_private_state"] = final_private_state
