...
class NatureCNN(nn.Module):
    def __init__(self, sample_obs):
        ...
        with torch.no_grad():
           ...
           # print("sample_obs[rgb].float().permute(0,3,1,2).cpu()", sample_obs["rgb_0"].float().permute(0,3,1,2).cpu().shape)
           # print("sample_obs[rgb].float().permute(0,3,1,2).cpu()",
           #       sample_obs["rgb_1"].float().permute(0, 3, 1, 2).cpu().shape)
           # assert 1==2
        ...
        with torch.no_grad():
          #  print("sample_obs[rgb].float().permute(0,3,1,2).cpu()", sample_obs["rgb"].float().permute(0,3,1,2).cpu().shape)
          #  assert 1==2
            n_flatten_2 = cnn_2(sample_obs["rgb_1"].float().permute(0,3,1,2).cpu()).shape[1]
            fc_2 = nn.Sequential(nn.Linear(n_flatten_2, feature_size), nn.ReLU())
        extractors["rgb_1"] = nn.Sequential(cnn_2, fc_2)
        self.out_features += feature_size
