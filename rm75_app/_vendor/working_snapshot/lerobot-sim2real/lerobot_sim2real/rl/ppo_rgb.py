...
class PPOArgs:
    ...
    control_mode: Optional[str] = "pd_joint_target_delta_pos" #None
    ...
    # ROI loss (net_type==4): GIoU 对框大小不敏感，默认等权即可
    roi_loss_weight_object: float = 1.0
    """weight for object ROI (index 1); gripper=1.0. 仅当确需偏重某一侧时可调，一般保持 1.0。"""
    roi_area_penalty_coef: float = 0.0
    """penalize pred area > gt（缓解「框很长」）. >0 时惩罚 area_ratio>1.5，推荐 0.05~0.15。"""
...
class DictArray(object):
    def __init__(self, buffer_shape, element_space, data_dict=None, device=None, pin_memory: bool = False):
        ...
        if data_dict:
            ...
            for k, v in element_space.items():
                if isinstance(v, gym.spaces.dict.Dict):
                    self.data[k] = DictArray(buffer_shape, v, device=device, pin_memory=pin_memory)
                else:
                    dtype = (torch.float32 if v.dtype in (np.float32, np.float64) else
                            ...
                            v.dtype)
                    shape = buffer_shape + v.shape
                    
                    # 关键：只有 CPU buffer 才能 pin
                    if (device is None or str(device) == "cpu") and pin_memory:
...
def _giou_loss(pred: torch.Tensor, target: torch.Tensor, eps: float = 1e-7, reduction: str = "mean") -> torch.Tensor:
    """
    GIoU loss for bboxes [B, N, 4] in (y_min, x_min, y_max, x_max). Scale-invariant.
    Returns 1 - GIoU, so minimize this loss.
    reduction: "mean" -> scalar; "none" -> [B, N] per-box loss (for per-ROI weighting).
    """
    pred = pred.float()
    target = target.float()
    # pred, target: [B, N, 4]
    py1, px1, py2, px2 = pred[..., 0], pred[..., 1], pred[..., 2], pred[..., 3]
    ty1, tx1, ty2, tx2 = target[..., 0], target[..., 1], target[..., 2], target[..., 3]
    area_p = (py2 - py1).clamp(min=0) * (px2 - px1).clamp(min=0)
    area_t = (ty2 - ty1).clamp(min=0) * (tx2 - tx1).clamp(min=0)
    iy1 = torch.maximum(py1, ty1)
    ix1 = torch.maximum(px1, tx1)
    iy2 = torch.minimum(py2, ty2)
    ix2 = torch.minimum(px2, tx2)
    inter = (iy2 - iy1).clamp(min=0) * (ix2 - ix1).clamp(min=0)
    ...
    area_c = (cy2 - cy1).clamp(min=0) * (cx2 - cx1).clamp(min=0) + eps
...
def _bboxes_to_roi_mask(bboxes: torch.Tensor, H: int, W: int) -> torch.Tensor:
    """从 bboxes [B, N, 4] 创建全分辨率 roi_mask [B,1,H,W]，完全 GPU 向量化，无 .item() 无 Python 循环。"""
    B, N, _ = bboxes.shape
    device = bboxes.device
    y0 = bboxes[..., 0].float().clamp(0, H - 1e-6)
    x0 = bboxes[..., 1].float().clamp(0, W - 1e-6)
    y1 = bboxes[..., 2].float().clamp(0, H)
    x1 = bboxes[..., 3].float().clamp(0, W)
    ...
    inside = (grid_y >= y0) & (grid_y < y1) & (grid_x >= x0) & (grid_x < x1)
...
class ROIPredictor(nn.Module):
    """
    从全图预测 ROI bbox。输入: x [B,C,H,W]  输出: bboxes [B, N_roi, 4] (y_min,x_min,y_max,x_max)
    
    改进点：
    1. 分离预测头：gripper 和 object 各自独立的 fc head，避免互相干扰
    2. 中心点+宽高参数化：预测 (cy, cx, h, w)，更稳定
    3. 宽高上限：限制框的最大尺寸，避免"框很长"
    """
    def __init__(self, in_ch: int, n_roi: int = 2, max_hw_ratio: float = 0.5):
        super().__init__()
        self.n_roi = n_roi
        self.max_hw_ratio = max_hw_ratio  # 框的最大宽高占图像的比例
        
        # 共享特征提取（最后用 AdaptiveAvgPool 所以 padding 不影响）
        ...
        # 分离预测头：每个 ROI 独立的 fc，输出 4 维 (cy, cx, h, w)
        self.heads = nn.ModuleList([
            nn.Sequential(nn.Flatten(), nn.Linear(feat_dim, 128), nn.ReLU(inplace=True), nn.Linear(128, 4))
    ...
    def forward(self, x_nchw: torch.Tensor) -> torch.Tensor:
        ...
        for head in self.heads:
            out = torch.sigmoid(head(h))  # [B, 4]: (cy, cx, h, w) 归一化到 [0,1]
            ...
            # 宽高：限制最大尺寸，最小 5% 图像尺寸
            box_h = (out[:, 2] * self.max_hw_ratio + 0.05) * H  # [0.05H, (max_hw_ratio+0.05)H]
            box_w = (out[:, 3] * self.max_hw_ratio + 0.05) * W
            
            # 转换为 (y_min, x_min, y_max, x_max)
            y_min = (cy - box_h / 2).clamp(min=0)
            x_min = (cx - box_w / 2).clamp(min=0)
            y_max = (cy + box_h / 2).clamp(max=H)
            x_max = (cx + box_w / 2).clamp(max=W)
            
            bbox = torch.stack([y_min, x_min, y_max, x_max], dim=-1)  # [B, 4]
            all_bboxes.append(bbox)
        
        pred_float = torch.stack(all_bboxes, dim=1)  # [B, N_roi, 4]
        pred_int = pred_float.round().long().clamp(min=0)
        self.last_pred_bboxes_float = pred_float
        return pred_int


class DualRoIAttnEncoderNCHW(nn.Module):
    """ROI 置黑非 ROI 区域，全分辨率 AttnEncoder 预测 mask。不 crop/resize。"""
    def __init__(self, in_ch: int, feature_size: int = 256, n_roi: int = 2):
        super().__init__()
        self.n_roi = n_roi
        self.roi_predictor = ROIPredictor(in_ch, n_roi)
        self.attn_encoder = AttnConvEncoderNCHW(in_ch, feature_size)

    def forward(self, x_nchw: torch.Tensor, gt_bboxes: torch.Tensor = None) -> torch.Tensor:
        B, _, H, W = x_nchw.shape
        pred_bboxes = self.roi_predictor(x_nchw)
        self.last_pred_bboxes = pred_bboxes
        self.last_gt_bboxes = gt_bboxes
        roi_mask = _bboxes_to_roi_mask(pred_bboxes, H, W)
        self.last_roi_mask = roi_mask  # 保存用于可视化
        rgb_roi = x_nchw * roi_mask
        self.last_rgb_roi = rgb_roi
        # 传入 roi_mask，让 attention 约束在 ROI 区域内（取交集）
        z = self.attn_encoder(rgb_roi, roi_mask_hw=roi_mask)
        self.last_attn = self.attn_encoder.last_attn
        self.last_attn_raw = getattr(self.attn_encoder, "last_attn_raw", None)  # 未约束的原始 attention
        self.last_roi_mask_small = getattr(self.attn_encoder, "last_roi_mask_small", None)
        self.last_attn_logits = getattr(self.attn_encoder, "last_attn_logits", None)
        return z
...
class AttnConvEncoderNCHW(nn.Module):
    ...
    roi_mask_hw: 可选，全分辨率 ROI mask [B,1,H,W]，用于约束 attention 只在 ROI 内
    
    添加 padding 使特征图空间位置与输入图像对齐：
    - 无 padding 时：128x128 → 12x12，上采样回去会有 ~4px 偏移
    - 有 padding 时：128x128 → 16x16，空间对齐更好（总 stride=8，输出=H/8）
    """
    def __init__(self, in_ch: int, feature_size: int = 256, use_padding: bool = True):
        super().__init__()
        # 添加 padding 使输出尺寸 = H/8，空间位置对齐更好
        # padding 计算：p = (k - s) // 2，使输出 ≈ ceil(input / stride)

        self.conv = nn.Sequential(
            nn.Conv2d(in_ch, 32, kernel_size=8, stride=4, padding=2), nn.ReLU(inplace=True),  # H/4
            nn.Conv2d(32, 64, kernel_size=4, stride=2, padding=1),    nn.ReLU(inplace=True),  # H/8
            nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1),    nn.ReLU(inplace=True),  # H/8
        )
        ...
        self.last_attn_raw = None  # 未经 ROI 约束的原始 attention
        self.last_roi_mask_small = None  # [B,1,H',W'] ROI mask 下采样到注意力分辨率

    def forward(self, x_nchw: torch.Tensor, roi_mask_hw: torch.Tensor = None) -> torch.Tensor:
        h = self.conv(x_nchw)                          # [B,64,H',W']
        logits = self.attn_head(h)
        A = torch.sigmoid(logits)           # [B,1,H',W']
        self.last_attn_raw = A
        self.last_attn_logits = logits
        
        # 如果提供了 ROI mask，将 attention 约束在 ROI 区域内
        if roi_mask_hw is not None:
            # 下采样 ROI mask 到特征图尺寸
            _, _, Hp, Wp = h.shape
            roi_mask_small = F.interpolate(roi_mask_hw, size=(Hp, Wp), mode='bilinear', align_corners=False)
            self.last_roi_mask_small = roi_mask_small
            # attention 与 ROI mask 取交集
            A = A * roi_mask_small
        else:
            self.last_roi_mask_small = None
...
class NatureCNN(nn.Module):
    def __init__(self, sample_obs, type="1"):
        ...
        if type == "1" : # total share - 单目相机，只用rgb_0
            ...
            self.out_features += feature_size
        elif type == "4":  # ROI 置黑 + 全分辨率 AttnEncoder
            extractors["rgb_0"] = DualRoIAttnEncoderNCHW(int(in_channels), feature_size)
            self.out_features += feature_size
            self._use_dual_roi = True
            self._dual_roi_keys = ("gripper", "object")
        else:
            assert type in ("1", "2", "3", "4"), f"Unknown type {type}"
        if type != "4":
            self._use_dual_roi = False
    ...
    def forward(self, observations) -> torch.Tensor:
        ...
        attn_logits = {}
        gt_bboxes = None
        if getattr(self, "_use_dual_roi", False):
            roi_keys = getattr(self, "_dual_roi_keys", ("gripper", "object"))
            bg = observations.get(f"roi_bbox_0_{roi_keys[0]}")
            bo = observations.get(f"roi_bbox_0_{roi_keys[1]}")
            if bg is not None and bo is not None:
                gt_bboxes = torch.stack([bg, bo], dim=1)
        for key, extractor in self.extractors.items():
            obs = observations[key]
            if "rgb" in key:
                x = obs.float().permute(0, 3, 1, 2) / 255.0  # NCHW, 0~1
                if getattr(self, "_use_dual_roi", False) and hasattr(extractor, "forward"):
                    z = extractor(x, gt_bboxes=gt_bboxes)
                else:
                    z = extractor(x)
...
class Agent(nn.Module):
    def __init__(self, method, sigma, envs, sample_obs, net_type, choice, tcl_choice, device,z_detach, private_dim, search_entropy, act_type, critic_use_pi=0):
        ...
        self.last_pred_bboxes = None
        self.last_gt_bboxes = None
        self.last_rgb_roi = None
    ...
    def get_action_and_value(self, x, action=None, x_next=None, done=None, private_obs=None, org_x = None, alpha = 0.0, adv=None):
        ...
        if hasattr(self.feature_net, "last_attn_logits"):
            ...
            if "rgb_0" in ml: self.last_attn_logits["rgb_0"] = ml["rgb_0"]
        if hasattr(self.feature_net, "extractors"):
            enc = self.feature_net.extractors["rgb_0"] if "rgb_0" in self.feature_net.extractors else None
            if enc is not None and hasattr(enc, "last_pred_bboxes"):
                self.last_pred_bboxes = enc.last_pred_bboxes
            if enc is not None and hasattr(enc, "last_gt_bboxes"):
                self.last_gt_bboxes = enc.last_gt_bboxes
            if enc is not None and hasattr(enc, "last_rgb_roi"):
                self.last_rgb_roi = enc.last_rgb_roi
...
def make_overlay(rgb_bhwc: torch.Tensor,
    ...
    if gt_b1hw is not None:
        M_up = F.interpolate(gt_b1hw, size=(H, W), mode='bilinear', align_corners=False).clamp(0,1)
...
class MaskDebugDumper:
    ...
    def write(self, step: int,
        ...
        if down_b1hw is not None:
            up = F.interpolate(down_b1hw, size=(H,W), mode="bilinear", align_corners=False)
...
def get_infos(env_id, infos):
    """
    根据环境ID提取私有信息（privileged information）
    ...
    Returns:
        private_info: 拼接后的私有信息tensor
    """
    if env_id == "SO101GraspCube_two_cameras-v1": # Done
        private_info = torch.cat(
            [infos["obj_pose"], infos["tcp_pose"], infos["tcp_to_obj_pos"]], -1)
    elif env_id == "RM75Soap_two_cameras-v1": # Done
        private_info = torch.cat(
            [infos["tcp_pose"], infos["pump_head_pos"], infos["tcp_to_pump_head_pos"]], -1)
    elif env_id == "RM75GraspCube_two_cameras-v1": # Done
        private_info = torch.cat(
            [infos["obj_pose"], infos["tcp_pose"], infos["tcp_to_obj_pos"]], -1)
    elif env_id == "RM75Hammer-v1":
        private_info = torch.cat(
            [infos["obj_pose"], infos["tcp_pose"], infos["tcp_to_obj_pos"]], -1)
    ...
    elif env_id == "SO101GraspYCB_color_return_two_cameras-v1":
        private_info = torch.cat(
            [infos["obj_pose"], infos["tcp_pose"], infos["obj_pos"], infos["tcp_to_obj_pos"]], -1)
        ...
        private_info = torch.cat(
            [infos["obj_pose"], infos["tcp_pose"], infos["obj_pos"], infos["tcp_to_obj_pos"]], -1)
    elif env_id == "SO101GraspVisdex_color_return_two_cameras-v1": # Done
        private_info = torch.cat(
            [infos["obj_pose"], infos["tcp_pose"], infos["obj_pos"], infos["tcp_to_obj_pos"]], -1)
    ...
    elif env_id == "RM75CloseBox-v1":
        private_info = torch.cat(
            [infos["box_pos"], infos["tcp_pos"], infos["tcp_to_lid_pos"], infos["lid_pos"]], -1)
    else:
        # 如果环境ID不匹配，抛出错误提示
        raise ValueError(f"Unsupported environment ID: {env_id}. Please add support for this environment in get_infos function.")
...
def train(args: PPOArgs):
    ...
    # 将 obs buffer 放在 CPU 上使用 pinned memory，以省显存并加速传输
    obs = DictArray((args.num_steps, args.num_envs), envs.single_observation_space, device=torch.device("cpu"), pin_memory=True)
    ...
    # 改动A：添加 copy_obs_into_pinned_buffer 函数
    def copy_obs_into_pinned_buffer(dst: DictArray, step: int, src_obs: dict):
        """
        将 GPU 上的 obs 直接 copy_ 写进预先分配好的 CPU pinned buffer
        
        Args:
            dst: obs DictArray（CPU pin_memory=True）
            step: 当前 step
            src_obs: envs.step() 返回的 next_obs（可能在 GPU）
        """
        for k in dst.keys():
            d = dst.data[k]
            s = src_obs[k]
            if isinstance(d, DictArray):
                # 遇到嵌套的 DictArray，继续递归（不做 [step]）
                copy_obs_into_pinned_buffer(d, step, s)
            else:
                # 只有叶子 tensor 才在这里取 step
                dst_slice = d[step]  # 叶子 tensor 才在这里取 step
                t = s
                if not isinstance(t, torch.Tensor):
                    raise TypeError(f"Unexpected type in obs[{k}]: {type(t)}")
                if t.dtype != dst_slice.dtype:
                    t = t.to(dtype=dst_slice.dtype)
                # dst_slice 是 pinned CPU tensor view，cuda->cpu 用 non_blocking 才能异步
                if t.is_cuda:
                    dst_slice.copy_(t, non_blocking=True)
                else:
                    dst_slice.copy_(t)
    
    # 辅助函数：智能设备转移（自动检测源设备并选择最佳传输方式）
    def move_obs_to_device(obs_dict, target_device, non_blocking=None):
        """
        将观察数据（dict）中的所有tensor转移到目标设备
        
        Args:
            obs_dict: 观察数据字典
            target_device: 目标设备
            non_blocking: 是否使用非阻塞传输
                - None: 自动判断（GPU->CPU pinned 或 CPU pinned->GPU 时使用 True）
                - True/False: 手动指定
        """
        if isinstance(obs_dict, dict):
            return {k: move_obs_to_device(v, target_device, non_blocking) for k, v in obs_dict.items()}
        elif isinstance(obs_dict, torch.Tensor):
            # 自动判断是否使用 non_blocking
            if non_blocking is None:
                # GPU -> CPU pinned 或 CPU pinned -> GPU 时使用 non_blocking
                src_is_cuda = obs_dict.device.type == "cuda"
                dst_is_cuda = target_device.type == "cuda"
                # 如果源或目标是 GPU，且目标/源是 CPU，使用 non_blocking
                use_non_blocking = (src_is_cuda and not dst_is_cuda) or (not src_is_cuda and dst_is_cuda)
            else:
                use_non_blocking = non_blocking
            return obs_dict.to(target_device, non_blocking=use_non_blocking)
        elif isinstance(obs_dict, (list, tuple)):
            return type(obs_dict)(move_obs_to_device(item, target_device, non_blocking) for item in obs_dict)
        else:
            return obs_dict
    
    def ensure_obs_on_device(obs_dict, target_device):
        """
        确保 obs 在目标设备上，只在需要时搬运（避免不必要的拷贝）
        ...
        Returns:
            确保在目标设备上的 obs_dict（如果已经在目标设备上，返回原对象）
        """
        # 检查是否有任何 tensor 不在目标设备上
        any_wrong = False
        for v in obs_dict.values():
            if isinstance(v, dict):
                # 递归检查嵌套 dict
                for sub_v in v.values():
                    if isinstance(sub_v, torch.Tensor) and sub_v.device != target_device:
                        any_wrong = True
                        break
                if any_wrong:
                    break
            elif isinstance(v, torch.Tensor) and v.device != target_device:
                any_wrong = True
                break
        
        # 只有在需要时才搬运（使用 non_blocking=True 加速）
        return move_obs_to_device(obs_dict, target_device, non_blocking=True) if any_wrong else obs_dict
    
    # 初始的 next_obs 可能在 GPU 上（如果 env 使用 GPU），转移到 CPU
    # next_obs = move_obs_to_device(next_obs, torch.device("cpu"))
    ...
    roi_bbox_gripper_buf = roi_bbox_object_buf = None
    if args.use_attn and args.evaluate is False:
        with torch.no_grad():
        ...
        # roi_debug_dir = os.path.join("runs", run_name, "roi_debug")
        # os.makedirs(roi_debug_dir, exist_ok=True)
        # x = next_obs["rgb_0"].float().permute(0, 3, 1, 2) / 255.0
        # if x.device.type != device.type:
        #     x = x.to(device)
        # B, C, H, W = x.shape
        # bg = infos["roi_bbox_0_gripper"].to(device) if isinstance(infos["roi_bbox_0_gripper"], torch.Tensor) else torch.tensor(infos["roi_bbox_0_gripper"], device=device)
        # bo = infos["roi_bbox_0_object"].to(device) if isinstance(infos["roi_bbox_0_object"], torch.Tensor) else torch.tensor(infos["roi_bbox_0_object"], device=device)
        # gt_bboxes = torch.stack([bg, bo], dim=1)
        # roi_mask_gt = _bboxes_to_roi_mask(gt_bboxes, H, W)
        # rgb_roi_gt = x * roi_mask_gt
        # img_gt = (rgb_roi_gt[0:1].permute(0, 2, 3, 1).clamp(0, 1).detach().cpu().numpy() * 255).astype(np.uint8)[0]
        # imageio.imwrite(os.path.join(roi_debug_dir, "rgb_roi_gt_env0.png"), img_gt)
        # rgb_roi = agent.last_rgb_roi
        # img = (rgb_roi[0:1].permute(0, 2, 3, 1).clamp(0, 1).detach().cpu().numpy() * 255).astype(np.uint8)[0]
        # imageio.imwrite(os.path.join(roi_debug_dir, "rgb_roi_pred_env0.png"), img)
        # mask_pred = agent.last_attn_maps["rgb_0"]
        # m = mask_pred[0:1].permute(0, 2, 3, 1).detach().cpu().numpy()
        # m_vis = (m * 255).astype(np.uint8)[0]
        # m_vis = np.repeat(m_vis, 3, axis=-1)
        # imageio.imwrite(os.path.join(roi_debug_dir, "mask_pred_env0.png"), m_vis)
        # mask_gt = infos["mask_0"]
        # m_gt = _ensure_B1HW_bin(mask_gt)[0:1].permute(0, 2, 3, 1).detach().cpu().numpy()
        # m_gt_vis = (m_gt * 255).astype(np.uint8)[0]
        # m_gt_vis = np.repeat(m_gt_vis, 3, axis=-1)
        # imageio.imwrite(os.path.join(roi_debug_dir, "mask_gt_env0.png"), m_gt_vis)
        # assert 1 == 2
        ...
        roi_bbox_gripper_buf = roi_bbox_object_buf = None
        if str(getattr(args, "net_type", "3")) == "4":
            roi_bbox_gripper_buf = torch.zeros((args.num_steps, args.num_envs, 4), device=device, dtype=torch.long)
            roi_bbox_object_buf = torch.zeros((args.num_steps, args.num_envs, 4), device=device, dtype=torch.long)
        if Aref is None:
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if iteration % args.eval_freq == 1:
            ...
            for eval_step in range(args.num_eval_steps):
                with torch.no_grad():
                    ...
                    action = agent.get_action(eval_obs, private_state=private_info, deterministic=True, alpha=alpha)
                    if args.evaluate and eval_step == 0:
                        roi_debug_dir = os.path.dirname(args.checkpoint) if args.checkpoint else "runs/roi_debug"
                        os.makedirs(roi_debug_dir, exist_ok=True)
                        rgb = eval_obs["rgb_0"]
                        ...
                        H, W = rgb.shape[:2]
                        pred_bboxes = None
                        extractors = getattr(agent.feature_net, "extractors", None)
                        enc = extractors["rgb_0"] if extractors is not None and "rgb_0" in extractors else None
                        if enc is not None and hasattr(enc, "last_pred_bboxes"):
                            pred_bboxes = enc.last_pred_bboxes[0].detach().cpu().numpy()
                            print(f"[ROI pred] gripper: {pred_bboxes[0].tolist()}  object: {pred_bboxes[1].tolist()}")
                        gt_bboxes = None
                        if "roi_bbox_0_gripper" in eval_infos and "roi_bbox_0_object" in eval_infos:
                            def _to_np_4(v):
                                if isinstance(v, torch.Tensor):
                                    v = v[0].cpu().numpy() if v.ndim > 1 else v.cpu().numpy()
                                else:
                                    v = np.array(v)
                                return v.reshape(4)
                            gt_bboxes = np.stack([_to_np_4(eval_infos["roi_bbox_0_gripper"]), _to_np_4(eval_infos["roi_bbox_0_object"])], axis=0)
                            print(f"[ROI GT]   gripper: {gt_bboxes[0].tolist()}  object: {gt_bboxes[1].tolist()}")
                        rgb_draw = rgb.copy()
                        for bboxes, colors in [(pred_bboxes, [(0, 255, 0), (255, 0, 0)]), (gt_bboxes, [(0, 255, 255), (255, 165, 0)])]:
                            if bboxes is not None:
                                for i, (y0, x0, y1, x1) in enumerate(bboxes[:2]):
                                    if y1 > y0 and x1 > x0:
                                        cv2.rectangle(rgb_draw, (int(x0), int(y0)), (int(x1), int(y1)), colors[i % 2], 2)
                        imageio.imwrite(os.path.join(roi_debug_dir, "roi_bboxes.png"), rgb_draw)
                        m0_gt = masks_from_infos(eval_infos, 0)
                        gt_mask_np = None
                        if m0_gt is not None:
                            gt_mask_np = F.interpolate(m0_gt, size=(H, W), mode="bilinear", align_corners=False)[0, 0].detach().cpu().numpy()
                        attn_maps = getattr(agent.feature_net, "last_attn_maps", {}) or {}
                        attn = attn_maps.get("rgb_0")
                        pred_attn_np = None
                        if attn is not None:
                            pred_attn_np = F.interpolate(attn, size=(H, W), mode="bilinear", align_corners=False)[0, 0].detach().cpu().numpy()
                        # 低分辨率 debug（希望看到 16x16 的 attention/logits/roi_mask_small）
                        pred_attn_raw_low_np = None   # sigmoid(logits) before ROI masking
                        pred_attn_logits_low_np = None
                        pred_roi_mask_small_np = None
                        if enc is not None:
                            attn_raw_low = getattr(enc, "last_attn_raw", None)
                            if attn_raw_low is not None:
                                pred_attn_raw_low_np = attn_raw_low[0, 0].detach().cpu().numpy()
                            attn_logits_low = getattr(enc, "last_attn_logits", None)
                            if attn_logits_low is not None:
                                pred_attn_logits_low_np = attn_logits_low[0, 0].detach().cpu().numpy()
                            roi_small = getattr(enc, "last_roi_mask_small", None)
                            if roi_small is not None:
                                pred_roi_mask_small_np = roi_small[0, 0].detach().cpu().numpy()
                        n_cols = 2
                        n_rows = 2
                        if pred_attn_np is not None or gt_mask_np is not None:
                            fig, axes = plt.subplots(n_rows, n_cols, figsize=(4 * n_cols, 4 * n_rows))
                            axes = np.atleast_2d(axes)
                            idx = 0
                            if pred_attn_np is not None:
                                ax = axes[idx // n_cols, idx % n_cols]
                                ax.imshow(pred_attn_np, cmap="jet")
                                ax.set_title("Pred Mask Heatmap")
                                ax.axis("off")
                                idx += 1
                            if gt_mask_np is not None:
                                ...
                                idx += 1
                            if pred_attn_np is not None:
                                an = (pred_attn_np - pred_attn_np.min()) / (pred_attn_np.max() - pred_attn_np.min() + 1e-8)
                                jet = plt.cm.jet(an)[..., :3]
                                overlay = (rgb.astype(np.float32) / 255 * 0.5 + jet * 0.5).clip(0, 1)
                                ax = axes[idx // n_cols, idx % n_cols]
                                ax.imshow(overlay)
                                ax.set_title("RGB + Pred Attn")
                                ax.axis("off")
                                idx += 1
                            if gt_mask_np is not None:
                                gt_overlay = rgb.copy().astype(np.float32) / 255
                                gt_overlay[..., 1] = np.maximum(gt_overlay[..., 1], 0.6 * gt_mask_np)
                                ax = axes[idx // n_cols, idx % n_cols]
                                ax.imshow(gt_overlay.clip(0, 1))
                                ax.set_title("RGB + GT Mask")
                                ax.axis("off")
                            plt.tight_layout()
                            plt.savefig(os.path.join(roi_debug_dir, "roi_and_mask.png"), dpi=120)
                            plt.close()
                        if pred_attn_raw_low_np is not None or pred_attn_logits_low_np is not None or pred_roi_mask_small_np is not None:
                            fig2, axes2 = plt.subplots(1, 3, figsize=(12, 4))
                            axes2 = np.atleast_1d(axes2)
                            if pred_attn_raw_low_np is not None:
                                axes2[0].imshow(pred_attn_raw_low_np, cmap="jet")
                                axes2[0].set_title(f"Attn raw sigmoid (pre-ROI) {pred_attn_raw_low_np.shape[0]}x{pred_attn_raw_low_np.shape[1]}")
                                axes2[0].axis("off")
                            else:
                                axes2[0].set_title("Attn raw sigmoid (N/A)")
                                axes2[0].axis("off")
                            if pred_attn_logits_low_np is not None:
                                axes2[1].imshow(pred_attn_logits_low_np, cmap="coolwarm")
                                axes2[1].set_title(f"Attn logits {pred_attn_logits_low_np.shape[0]}x{pred_attn_logits_low_np.shape[1]}")
                                axes2[1].axis("off")
                            else:
                                axes2[1].set_title("Attn logits (N/A)")
                                axes2[1].axis("off")
                            if pred_roi_mask_small_np is not None:
                                axes2[2].imshow(pred_roi_mask_small_np, cmap="gray", vmin=0, vmax=1)
                                axes2[2].set_title(f"ROI mask small {pred_roi_mask_small_np.shape[0]}x{pred_roi_mask_small_np.shape[1]}")
                                axes2[2].axis("off")
                            else:
                                axes2[2].set_title("ROI mask small (N/A)")
                                axes2[2].axis("off")
                            plt.tight_layout()
                            plt.savefig(os.path.join(roi_debug_dir, "attn_lowres_debug.png"), dpi=140)
                            plt.close()
                        print(f"[ROI+Mask] 已保存到 {roi_debug_dir}/  (绿/红=pred ROI框, 青/橙=GT ROI框; 另存 attn_lowres_debug.png)")
                        assert 1 == 2, "ROI+mask 可视化已保存，assert 停止"
        ...
        # 改动B：使用 copy_obs_into_pinned_buffer 替代 move_obs_to_device
        # 使用 copy_obs_into_pinned_buffer 直接 copy 到预先分配好的 CPU pinned buffer
        # 内部已经使用 non_blocking=True，在默认 stream 上异步执行
        for step in range(0, args.num_steps):
            global_step += args.num_envs
            # 将 next_obs 直接 copy_ 到预先分配好的 CPU pinned buffer
            # 不使用 to("cpu") 创建新 tensor，避免性能损失和数据不一致
            # copy_obs_into_pinned_buffer 内部对 cuda tensor 已经使用 non_blocking=True
            copy_obs_into_pinned_buffer(obs, step, next_obs)
        #    org_obs[step] = next_obs
            ...
            if roi_bbox_gripper_buf is not None and "roi_bbox_0_gripper" in infos:
                roi_bbox_gripper_buf[step] = infos["roi_bbox_0_gripper"].to(device)
                roi_bbox_object_buf[step] = infos["roi_bbox_0_object"].to(device)
            elif roi_bbox_gripper_buf is not None and not _roi_diag_done and step == 0 and iteration == 1:
                print("[ROI] 警告: infos 中无 roi_bbox_0_gripper，ROI loss 将恒为 0。请确认环境 (base_env) 已调用 register_roi 且 rgb_overlay_mode!='none'")
                _roi_diag_done = True
        ...
        #    org_obs[step]["rgb_0"] = infos["org_rgb_0"]
            ...
            with torch.no_grad():
                # 确保 next_obs 在正确的设备上（只在需要时搬运，避免不必要的拷贝）
                obs_for_policy = ensure_obs_on_device(next_obs, device)
                action, logprob, _, value, _, _ = agent.get_action_and_value(obs_for_policy, private_obs = private_obs[step], alpha = alpha)
                values[step] = value.flatten()
            actions[step] = action
            logprobs[step] = logprob

            # TRY NOT TO MODIFY: execute the game and log data.
            # envs.step() 返回的 next_obs 可能在 GPU 上（如果使用 sim_backend="physx_cuda"）
...
#            assert  1== 2
            ...
            if "final_info" in infos:
                ...
                with torch.no_grad():
                    # final_observation 可能在 CPU 上，需要转移到 GPU
                    final_obs_gpu = move_obs_to_device(infos["final_observation"], device)
                    final_values[step, torch.arange(args.num_envs, device=device)[done_mask]] = agent.get_value(final_obs_gpu, infos["final_private_state"]).view(-1)
        ...
        with torch.no_grad():
            # 确保 next_obs 在正确的设备上（最后一次使用 next_obs）
            next_obs_for_value = ensure_obs_on_device(next_obs, device)
            next_value = agent.get_value(next_obs_for_value, next_private_state).reshape(1, -1)
        ...
        # 在 rollout 结束、开始 update 之前，等待所有 GPU->CPU copy 完成
        # 使用简单的 torch.cuda.synchronize() 确保所有异步 copy 完成
        if device.type == "cuda":
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                mb_obs = move_obs_to_device(b_obs[mb_inds], device)
                if roi_bbox_gripper_buf is not None and isinstance(mb_obs, dict):
                    b_roi_g = roi_bbox_gripper_buf.reshape(-1, 4)
                    b_roi_o = roi_bbox_object_buf.reshape(-1, 4)
                    mb_obs = {**mb_obs, "roi_bbox_0_gripper": b_roi_g[mb_inds], "roi_bbox_0_object": b_roi_o[mb_inds]}
                _, newlogprob, entropy, newvalue, tcl_loss, plot_info  = agent.get_action_and_value(mb_obs, b_actions[mb_inds], None, b_next_done[mb_inds], b_private_obs[mb_inds], None, alpha = alpha, adv = mb_advantages)


                L_mask = torch.tensor(0.0, device=device)
                L_roi = torch.tensor(0.0, device=device)
                if attn_sup0 is not None:
                    last_attn_logits = getattr(agent, "last_attn_logits", {})
                    logits0 = last_attn_logits.get("rgb_0", None) if isinstance(last_attn_logits, dict) else last_attn_logits
                    ...
                    if logits0 is not None and isinstance(logits0, torch.Tensor):
                        b_sup0 = b_attn_sup0[mb_inds]
                        L_mask = _bce_logits(logits0, b_sup0)
                    # ROIPredictor 仅由 L_roi 更新；policy/value 不反传至 ROI（bbox→mask 为整数不可微）
                    if str(getattr(args, "net_type", "3")) == "4":
                        enc = getattr(agent.feature_net, "extractors", None)
                        rgb_enc = enc["rgb_0"] if enc is not None and "rgb_0" in enc else None
                        roi_pred = getattr(rgb_enc, "roi_predictor", None)
                        pred_float = getattr(roi_pred, "last_pred_bboxes_float", None) if roi_pred is not None else None
                        gt_bboxes = getattr(agent, "last_gt_bboxes", None)
                        if pred_float is not None and gt_bboxes is not None:
                            # GIoU 对框大小不敏感，默认等权；可选 per-ROI 权重仅当需要偏重一侧时用
                            w_obj = getattr(args, "roi_loss_weight_object", 1.0)
                            loss_per_box = _giou_loss(pred_float, gt_bboxes.float(), reduction="none")
                            n_roi = loss_per_box.shape[1]
                            w0 = 1.0
                            if n_roi >= 2 and (w0 != 1.0 or w_obj != 1.0):
                                L_roi = (w0 * loss_per_box[:, 0].mean() + w_obj * loss_per_box[:, 1].mean()) / (w0 + w_obj)
                            else:
                                L_roi = loss_per_box.mean()
                            # 惩罚预测框面积远大于 GT，才是缓解“框很长”
                            area_pen = getattr(args, "roi_area_penalty_coef", 0.0)
                            if area_pen > 0:
                                p, g = pred_float.float(), gt_bboxes.float()
                                area_p = (p[..., 2] - p[..., 0]).clamp(min=0) * (p[..., 3] - p[..., 1]).clamp(min=0)
                                area_t = (g[..., 2] - g[..., 0]).clamp(min=0) * (g[..., 3] - g[..., 1]).clamp(min=0) + 1e-7
                                ratio = area_p / area_t
                                L_roi = L_roi + area_pen * F.relu(ratio - 1.5).mean()
                        elif not _roi_diag_done:
                            print(f"[ROI] L_roi=0 原因: pred_float={pred_float is not None}, gt_bboxes={gt_bboxes is not None}, mb_obs有roi_bbox={mb_obs.get('roi_bbox_0_gripper') is not None if isinstance(mb_obs, dict) else 'N/A'}")
                            _roi_diag_done = True
                ...
                loss = L_mask + L_roi + args.tcl_weight * tcl_loss + pg_loss - args.ent_coef * entropy_loss + v_loss * args.vf_coef
        ...
        if str(getattr(args, "net_type", "3")) == "4":
            logger.add_scalar("losses/roi_loss", L_roi.item() if isinstance(L_roi, torch.Tensor) else float(L_roi), global_step)
