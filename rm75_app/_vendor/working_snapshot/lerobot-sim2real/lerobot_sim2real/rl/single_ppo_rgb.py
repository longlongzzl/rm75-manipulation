...
class NatureCNN(nn.Module):
    def __init__(self, device, sample_obs):
        super().__init__()
        self.device = device
        extractors = {}
    ...
    def forward(self, observations) -> torch.Tensor:
        ...
        for key, extractor in self.extractors.items():
            ...
            if key == "rgb":
                ...
                obs = obs / 255
            encoded_tensor_list.append(extractor(obs.to(self.device)))
        return torch.cat(encoded_tensor_list, dim=1)

class Agent(nn.Module):
    def __init__(self, envs, device, sample_obs):
        super().__init__()
        self.device = device
        self.feature_net = NatureCNN(device,sample_obs=sample_obs)
