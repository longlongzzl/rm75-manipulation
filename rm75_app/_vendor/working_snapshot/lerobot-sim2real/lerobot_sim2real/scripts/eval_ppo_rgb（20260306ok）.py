...
# ##############################
    ...
    agent = Agent(None, 0.2,  sim_env, sample_obs=real_obs, net_type="4", choice=10, tcl_choice=5, device= device, z_detach = False, private_dim= private_dim, search_entropy= 0.0, act_type=1,  critic_use_pi=True)
    if args.checkpoint:
    ...
    current_qpos_list = []
    #
    action_lsit =[ torch.tensor([[ 2.3885,  1.3168,  3.3737, -2.5864,  0.7209, -1.6087,  2.1273,  2.6310]],
       device='cuda:0',), torch.tensor([[ 2.2546, -0.2913,  2.5696, -1.9459,  1.1981,  1.6814,  2.4697,  2.9169]],
       device='cuda:0',), torch.tensor([[ 2.5723, -0.8813,  1.0293, -1.7376,  2.6783,  2.2935,  1.3417,  2.6207]],
       device='cuda:0',), torch.tensor([[ 2.6025, -1.4845,  0.2959, -1.9659,  3.4791,  2.6726,  0.3992,  2.5834]],
       device='cuda:0',), torch.tensor([[ 2.9141, -1.1869,  0.3279, -2.1005,  3.8248,  2.3681, -0.4771,  2.5607]],
       device='cuda:0',), torch.tensor([[ 2.3537, -0.9236,  0.9392, -1.9592,  3.3523,  1.9482, -0.4580,  2.3871]],
       device='cuda:0',), torch.tensor([[ 2.6119, -1.0934,  0.7133, -2.1522,  3.3370,  1.8315, -1.6006,  2.3467]],
       device='cuda:0',), torch.tensor([[ 1.5553, -1.1033,  0.6246, -2.0270,  2.5652,  1.1016, -1.4452,  2.1709]],
       device='cuda:0',), torch.tensor([[ 1.0484, -1.3361, -0.2252, -2.4251,  3.2751,  1.1204, -2.2187,  2.2779]],
       device='cuda:0',), torch.tensor([[ -0.6766, -1.2186, -0.9323, -2.4621,  1.9202,  0.7074, -1.8560,  2.4550]],
       device='cuda:0',), torch.tensor([[-1.3487,  0.3460, -1.2203, -2.7232,  1.7106, -0.6477, -3.8826,  2.7858]],
       device='cuda:0',), torch.tensor([[-1.6431,  1.1095, -1.1693, -2.0760, -1.0404, -0.7221, -2.4047,  2.1970]],
       device='cuda:0',), torch.tensor([[-0.9665,  0.5984, -0.5642, -0.2473,  0.3070, -1.0526, -1.2258,  0.1516]],
       device='cuda:0',), torch.tensor([[-0.9894,  0.8853, -0.4710, -0.1186,  0.5582, -0.9050, -1.3184, 0.0011819]],
       device='cuda:0',), torch.tensor([[-0.9406,  1.0729, -0.2058, -0.0765,  0.5771, -0.8820, -1.2998, 0.0146]],
       device='cuda:0',), torch.tensor([[-0.9473,  1.2386, -0.0388,  0.0878,  0.4498, -0.8686, -1.2264,  0.0121]],
       device='cuda:0',), torch.tensor([[-0.8547,  1.2649,  0.0769,  0.1339,  0.2611, -0.8132, -0.8627,  0.0047]],
       device='cuda:0',), torch.tensor([[-0.5816,  1.2804, -0.0369,  0.0939,  0.0568, -0.8481, -0.5981,  0.0112]],
       device='cuda:0',), torch.tensor([[-0.2792,  1.3256, -0.0915,  0.0624,  0.1163, -0.8468, -0.2382,  0.0136]],
       device='cuda:0',), torch.tensor([[-3.5463e-02,  1.3151e+00, -1.2931e-01, -1.8436e-02,  1.0788e-01,
         -7.4636e-01,  8.6766e-02, 0.0176]], device='cuda:0',)]
    ...
    while args.num_episodes is None or episode_count < args.num_episodes:
        ...
        for _ in tqdm(range(args.max_episode_steps)):
            ...
            # if args.use_real_robot:
            #     try:
            #         real_qpos_tensor = real_env.agent.robot.get_qpos()
            #         real_qpos_flat = real_qpos_tensor.detach().cpu().flatten()
            #         if real_qpos_flat.numel() >= 8:
            #             gripper_rad = float(real_qpos_flat[7])
            #             print(f"[RealObs] gripper qpos (rad): {gripper_rad:.4f}")
            #         else:
            #             print("[RealObs] gripper qpos unavailable in tensor:", real_qpos_flat.tolist())
            #     except Exception as exc:
            #         print("[RealObs] Failed to read real qpos for logging:", exc)
            # print("time1",time.time() - mt1)
            action = agent.get_action(agent_obs, None, deterministic=True)
            print("eval_ppo_rgb get action", action)
            # action_lsit.append(action)



            # action = action_lsit[_]
          #  print(args.use_real_robot, _ , "Get action!!!!::::::", action)
            attn_maps = {}
          ...
          #  print("action (deg)", torch.rad2deg(action).detach().cpu())
            if step_idx < 5:
                rgb0 = real_obs["rgb_0"]
                if isinstance(rgb0, torch.Tensor):
                    rgb0 = rgb0.detach().cpu().numpy()
                rgb0 = np.squeeze(rgb0, axis=0)
                if rgb0.ndim == 3 and rgb0.shape[0] in (3, 4):
                    rgb0 = rgb0.transpose(1, 2, 0)
                debug_img = np.clip(rgb0, 0, 255).astype(np.uint8)
                imageio.imwrite(f"debug_rgb0_step{step_idx}.png", debug_img)
        ...
        #输出差值
            if args.use_real_robot:
                ...
                if joint_keys is None:
                    joint_keys = [f"joint{i + 1}.pos" for i in range(sim_robot.get_qpos().shape[-1])]
                joint_names = [k.replace(".pos", "") for k in joint_keys]
                sim_qpos = sim_robot.get_qpos().detach().cpu().flatten().numpy()
                obs_qpos_rad = sim_qpos[: len(joint_keys)]

            target_qpos_rad = real_env.agent.controller.articulation.drive_targets[0, :len(joint_keys)].detach().cpu().numpy()
            diff_rad = target_qpos_rad - obs_qpos_rad
            print("tracking diff (rad):", diff_rad)
            print("tracking diff (deg):", np.rad2deg(diff_rad))

            qpos_list.append(action.detach().cpu().numpy().squeeze())
            # sim_env.render()


            ctrl = real_env.agent.controller  # 这是 CombinedController
            arm_ctrl = ctrl.controllers["arm"]  # 这是 PDJointPosController（或其子类）

            arm_target_qpos = arm_ctrl._target_qpos  # shape: (num_envs, arm_dof)
            # 单环境通常取第 0 个
            arm_target_qpos0 = arm_target_qpos[0].detach().cpu().numpy()

            drive_target_list.append(arm_target_qpos0)
            current_qpos_list.append(real_env.agent.controller.articulation.drive_targets)
            if _ == 9:
                # print("FInal drive_target_list")
                # print(drive_target_list)
                # print("current_qpos_list")
                # print(current_qpos_list)
                print("action list" ,action_lsit)
                raise SystemExit("停止于第 10 轮用于调试")
            # print(sim_obs)
            # print("arm_target_qpos0:",arm_target_qpos0)
            # print("real_env.agent.controller.articulation.drive_targets::::::::::",real_env.agent.controller.articulation.drive_targets)
            # # ========== 提取注意力热图 ==========
            # assert 1==2
            # TODO real
            # arm_target_qpos0: [1.6706872 - 0.08615954  0.02739515 - 1.5062435   0.0398475 - 1.4570258
            #                    1.0983702]
            # real_env.agent.controller.articulation.drive_targets:::::::::: tensor(
            #     [[1.6707, -0.0862, 0.0274, -1.5062, 0.0398, -1.4570, 1.0984, 0.0633,
            #       0.0000, 0.0633, 0.0000, 0.0000, 0.0000]])
            # TODO sim:
            # arm_target_qpos0: [1.6128314 - 0.04854617  0.03602321 - 1.548445    0.03508979 - 1.5310434
            #                    1.0747842]
            # real_env.agent.controller.articulation.drive_targets:::::::::: tensor(
            #     [[1.6128, -0.0485, 0.0360, -1.5484, 0.0351, -1.5310, 1.0748, 0.1307,
            #       0.0000, 0.1307, 0.0000, 0.0000, 0.0000]])

            # ##############测试
            # time.sleep(0.5)
            if args.use_real_robot==False:
                sim_qpos_full = real_env.unwrapped.agent.robot.get_qpos().detach().cpu().flatten().numpy()
            #    print("Sim qpos :", sim_qpos_full)
                sim_gripper_rad = sim_env.agent.robot.get_qpos()[0, 7].item()
                sim_gripper_norm = sim_gripper_rad / 0.91
            #    print("Sim gripper: rad =", sim_gripper_rad, "norm =", sim_gripper_norm)
            # sim_qpos_deg = np.rad2deg(sim_qpos_full)
            # print("Sim qpos (deg):", sim_qpos_deg.tolist())
            # print("")
            # time.sleep(0.5)
            #
            # obs = real_robot.get_observation()
            # joint_keys = [f"{jn}.pos" for jn in real_robot._joint_names]  # 顺序和 obs 构造一致
            # real_qpos = [obs[k] for k in joint_keys]
            # print("Real qpos (deg):", real_qpos)
            # print("Delta first7 (deg):", (sim_qpos_deg[:7] - np.array(real_qpos[:7])).tolist())

            sim_articulation = sim_env.unwrapped.agent.controller.articulation.drive_targets

            current_sim_qpos = sim_env.unwrapped.agent.robot.get_qpos().detach().cpu().flatten().numpy()
