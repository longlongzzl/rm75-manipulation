...
def overlay_envs(sim_env, real_env, undistort_fn=None):
    """
    ...
    for name in overlaid_dict:
        real_img = to_numpy_image(real_obs[name]["rgb"][0]) / 255.0
        sim_img = to_numpy_image(overlaid_dict[name]["rgb"][0]) / 255.0
        overlaid_imgs.append(0.5 * real_img + 0.5 * sim_img)
        sim_imgs_list.append(sim_img)
        real_imgs_list.append(real_img)
        loss = np.mean((real_img - sim_img) ** 2)
        print("current loss", loss)

    return tile_images(overlaid_imgs), tile_images(sim_imgs_list), tile_images(real_imgs_list)


def get_real_image(real_env, undistort_fn=None):
    real_obs = real_env.get_obs()["sensor_data"]

    real_imgs = [to_numpy_image(real_obs["base_camera"]["rgb"][0]) / 255.0]
...
def update_camera(sim_env):
    ...
    if "w" in active_keys:
        camera_offset[0] -= 0.0025  # Move forward
    if "s" in active_keys:
        camera_offset[0]  += 0.0025  # Move back
    if "d" in active_keys:
        camera_offset[1]  += 0.0025  # Move right
    if "a" in active_keys:
        camera_offset[1] -= 0.0025  # Move left
    if "up" in active_keys:
        camera_offset[2]  += 0.0025  # Move up
    if "down" in active_keys:
        camera_offset[2] -= 0.0025  # Move down

    camera_target_offset = torch.zeros(3, dtype=torch.float32)

    if "y" in active_keys:
        camera_target_offset[0] -= 0.0025 # Move forward
    if "h" in active_keys:
        camera_target_offset[0] += 0.0025 # Move back
    if "u" in active_keys:
        camera_target_offset[1] += 0.0025  # Move right
    if "j" in active_keys:
        camera_target_offset[1] -= 0.0025  # Move left
    if "i" in active_keys:
        camera_target_offset[2] += 0.0025  # Move right
    if "k" in active_keys:
        camera_target_offset[2] -= 0.0025  # Move left
...
MOVEMENT_SPEED = 0.8  # units per second
FOV_CHANGE_SPEED = 0.8  # radians per second
help_message_printed = False  # Flag to track if we've printed the help message


def _ensure_float_camera_settings(settings: dict) -> dict:
    if not isinstance(settings, dict):
        return settings
    for key in ("pos", "target"):
        if key in settings and isinstance(settings[key], (list, tuple)):
            settings[key] = [float(x) for x in settings[key]]
    if "fov" in settings:
...
def main(args: Args):
    ...
    if "base_camera_settings" in env_kwargs:
        env_kwargs["base_camera_settings"] = _ensure_float_camera_settings(env_kwargs["base_camera_settings"])
    if "base_camera_2_settings" in env_kwargs:
        env_kwargs["base_camera_2_settings"] = _ensure_float_camera_settings(env_kwargs["base_camera_2_settings"])
    ...
    # ======== DEBUG: coordinate frames sanity check ========
    import numpy as np

    def _pose_to_pq(pose):
        # SAPIEN Pose has .p (xyz) and .q (wxyz) typically
        p = np.array(pose.p, dtype=float)
        q = np.array(pose.q, dtype=float)
        return p, q

    def _print_pose(name, pose):
        p, q = _pose_to_pq(pose)
        print(f"[frame] {name}: p={p.tolist()} q={q.tolist()}")

    sim_u = sim_env.unwrapped

    print("\n===== COORD CHECK (sim) =====")
    # 1) robot base in world
    try:
        _print_pose("world_T_robot_base", sim_env.agent.robot.pose)
    except Exception as e:
        print("[frame] world_T_robot_base: <failed>", e)

    # 2) tcp in world (if available)
    for attr in ["tcp_pose", "ee_pose", "tool_pose"]:
        if hasattr(sim_env.agent, attr):
            try:
                _print_pose(f"world_T_{attr}", getattr(sim_env.agent, attr))
            except Exception as e:
                print(f"[frame] world_T_{attr}: <failed>", e)

    # 3) camera mount in world (this is what you move)
    if hasattr(sim_u, "camera_mount"):
        try:
            _print_pose("world_T_camera_mount", sim_u.camera_mount.pose)
        except Exception as e:
            print("[frame] world_T_camera_mount: <failed>", e)

    # 4) base_camera_settings as used by your script
    if hasattr(sim_u, "base_camera_settings"):
        print("[cfg] base_camera_settings =", sim_u.base_camera_settings)

    # 5) actual sensor camera pose (may differ from mount depending on hierarchy)
    try:
        cam = sim_u._sensors["base_camera"].camera
        # many camera objects have .get_pose() / .pose; try both
        if hasattr(cam, "pose"):
            _print_pose("world_T_base_camera(sensor).pose", cam.pose)
        elif hasattr(cam, "get_pose"):
            _print_pose("world_T_base_camera(sensor).get_pose()", cam.get_pose())
    except Exception as e:
        print("[frame] world_T_base_camera(sensor): <failed>", e)

    print("===== END COORD CHECK =====\n")
    # =======================================================

    undistort_fn = build_undistort_fn(args.intrinsics_yaml)
...
#    fig.show(block=False)  # 非阻塞显示
   ...
   # real_agent.set_target_qpos(fixed_qpos)
    ...
    overlaid_imgs, sim_imgs, real_imgs = overlay_envs(sim_env, real_env, undistort_fn)
