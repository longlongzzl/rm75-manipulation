...
def main(args: Args):
 ...
 #    real_robot.disconnect()
    ...
    # 例：肩（shoulder_lift_link）-肘（elbow_link）-腕（wrist_flex_link）的夹角
    ...
    def link_axis_world(link, axis="x", *, device="cpu", dtype=torch.float32):
        """
        返回 link 的局部 <axis> 轴在世界坐标系下的方向，shape = (3,)
        兼容 ManiSkill3 (N,4,4) 齐次矩阵输出
        """
        # (N,4,4) → squeeze 去掉 batch, 然后取旋转子矩阵
        R = link.pose.to_transformation_matrix().squeeze(0)[:3, :3]  # (3,3) torch
        v_world = torch.matmul(R, torch.tensor(AXES[axis]).to(device))  # (3,)
        return v_world.to(dtype=dtype)




    while True:
        upper = env.unwrapped.agent.robot.links_map["Wrist_Pitch_Roll"]
        lower = env.unwrapped.agent.robot.links_map["Lower_Arm"]

        # 选用局部 x 轴，也可以换成 "y" / "z"
        v_upper = link_axis_world(upper, axis="x")
        v_lower = link_axis_world(lower, axis="x")

        # 夹角公式： arccos( (u·v) / (||u||·||v||) )
        cos_theta = torch.dot(v_upper, v_lower) / (
                torch.linalg.norm(v_upper) * torch.linalg.norm(v_lower)
        )
        theta = torch.acos(torch.clamp(cos_theta, -1.0, 1.0))

        print(f"Upper_Arm ↔ Lower_Arm around local-x: {math.degrees(theta):.2f}°", np.deg2rad(math.degrees(theta)-180))
      # real_robot.set_target_qpos(qpos * 0.0)
