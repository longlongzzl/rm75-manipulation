...
class ClippedLeRobotRealAgent(LeRobotRealAgent):
    ...
    def __init__(self, robot, use_cached_qpos=True, max_retries=3, retry_delay=0.1, **kwargs):
        ...
        self._last_gripper_command_rad = None
        super().__init__(robot, use_cached_qpos=use_cached_qpos, **kwargs)
        self._uses_le_robot_bus = hasattr(self.real_robot, "bus")
        self._missing_obs_warned = set()

    def set_gripper_limits(self, limits_rad):
        self._gripper_limits_rad = tuple(float(x) for x in limits_rad)
        if self._last_gripper_command_rad is None and self._gripper_limits_rad is not None:
            # Default to the midpoint when we haven't commanded anything yet.
            lo, hi = self._gripper_limits_rad
            self._last_gripper_command_rad = float(0.5 * (lo + hi))

    def _record_gripper_command(self, value_rad):
        if value_rad is None:
            return
        self._last_gripper_command_rad = float(value_rad)

    def _convert_obs_value(self, key, value, value_in_rad=False):
        """Convert an observation entry to radians, handling gripper normalization."""
        if value_in_rad:
            return float(value)
    ...
    def _last_gripper_norm(self):
        if self._gripper_limits_rad is None or self._last_gripper_command_rad is None:
            return None
        lo, hi = self._gripper_limits_rad
        span = max(hi - lo, 1e-6)
        return float(np.clip((self._last_gripper_command_rad - lo) / span, 0.0, 1.0))

    def _to_realman_gripper_delta(self, desired_norm):
        last_norm = self._last_gripper_norm()
        if last_norm is None:
            return desired_norm
        delta = float(desired_norm - last_norm)
    ...
    def set_target_qpos(self, qpos):
        ...
        if self._uses_le_robot_bus:
            ...
            if gripper_value is not None:
                ...
                try:
                    ...
                    self._record_gripper_command(gripper_value)
                except Exception as exc:
        ...
        use_degrees = getattr(self.real_robot.config, "use_degrees", False)
        action = {}
        for i, key in enumerate(self._motor_keys):
            if i >= qpos_tensor.shape[0]:
                break
            raw_rad = float(qpos_tensor[i])
            value = raw_rad
            if key.startswith("gripper"):
                if self._gripper_limits_rad is not None:
                    lo, hi = self._gripper_limits_rad
                    span = max(hi - lo, 1e-6)
                    value = (raw_rad - lo) / span
                    value = float(np.clip(value, 0.0, 1.0))
                elif use_degrees:
                    value = float(np.rad2deg(raw_rad))
                value = self._to_realman_gripper_delta(value)
                self._record_gripper_command(raw_rad)
            else:
                if use_degrees:
                    value = float(np.rad2deg(raw_rad))
            action[key] = value
    ...
    def get_qpos(self):
        ...
        if self._cached_qpos is not None:
            cached = self._cached_qpos.detach().cpu().flatten().numpy()
        values = []
        for idx, key in enumerate(ordered):
            raw_val = float(obs.get(key, float("nan")))
            value_in_rad = False
            used_fallback = False
            if not np.isfinite(raw_val):
                fallback = None
                if key.startswith("gripper") and self._last_gripper_command_rad is not None:
                    fallback = self._last_gripper_command_rad
                elif cached is not None and idx < len(cached):
                    fallback = float(cached[idx])
                if fallback is None:
                    fallback = 0.0
                raw_val = fallback
                value_in_rad = True
                used_fallback = True
                if key not in self._missing_obs_warned:
                    print(f"[WARN] Missing finite value for {key} in real observation; using fallback.")
                    self._missing_obs_warned.add(key)
            converted = self._convert_obs_value(key, raw_val, value_in_rad=value_in_rad)
            values.append(converted)
            if key.startswith("gripper") and not used_fallback:
                self._record_gripper_command(converted)
    ...
    def apply_sim_qpos(self, sim_qpos, override_gripper_norm=None):
        ...
        commanded_gripper_rad = None
        if drive_limit is not None:
            commanded_gripper_rad = lo + gripper_val * span
        elif sim_qpos.size > 0:
            commanded_gripper_rad = float(sim_qpos[drive_idx])
        ...
        send_gripper = gripper_val
        if self._gripper_limits_rad is not None:
            send_gripper = self._to_realman_gripper_delta(gripper_val)
        action["gripper.pos"] = send_gripper
        self.real_robot.send_action(action)
        self._record_gripper_command(commanded_gripper_rad)
...
class ActionTuner:
    def __init__(self, org_pos, dim, gripper_index=None, limits_deg=None, limits_rad=None):
        ...
        for i in range(self.dim):
            ...
            if self.gripper_index is not None and i == self.gripper_index:
                ...
                init_val = float(np.clip(init_val, lo, hi))
                self.vals.append(tk.DoubleVar(value=init_val))
            else:
    ...
    def set_values(self, values):
        ...
        for i in range(self.dim):
            if self.gripper_index is not None and i == self.gripper_index:
                lo, hi = self._get_limits_rad(i)
                self.vals[i].set(float(np.clip(padded[i], lo, hi)))
            else:
    ...
    def update_readback(self, real_values=None):
        ...
        for i in range(self.dim):
            if self.gripper_index is not None and i == self.gripper_index:
                desired_val = float(self.vals[i].get())
                actual_val = float(arr[i]) if arr is not None and i < len(arr) else None
                target_text = f"目标 {desired_val:.3f}"
                actual_text = f"实际 {actual_val:.3f}" if actual_val is not None else "实际 --"
            else:
