...
def main(args: Args):
    ...
    print("real_obs", real_obs.keys())
    rgb_img = real_obs["rgb"].cpu().numpy()[0].astype(np.uint8)[:,:, :3]
    # rgb_img_2 = real_obs["rgb"].cpu().numpy()[0].astype(np.uint8)[:, :, 3:]
   # plt.show(rgb_img)
    print(rgb_img.shape)
    bgr_img = cv2.cvtColor(rgb_img, cv2.COLOR_RGB2BGR)
    # bgr_img_2 = cv2.cvtColor(rgb_img_2, cv2.COLOR_RGB2BGR)
    # Save the image
    cv2.imwrite(args.out, bgr_img)

    # cv2.imwrite("greenscreen_2.png", bgr_img_2)
