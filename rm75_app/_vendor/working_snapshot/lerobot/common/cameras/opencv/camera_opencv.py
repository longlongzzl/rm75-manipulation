...
class OpenCVCamera(Camera):
    ...
    def _validate_fps(self) -> None:
        ...
        # Some Linux drivers report failure even though the fps is already correct,
        # so only warn if the effective fps is outside tolerance.
        if not math.isclose(self.fps, actual_fps, rel_tol=1e-2, abs_tol=1e-2):
            logger.warning(
                "%s failed to set fps=%s (actual_fps=%s, success=%s). "
                "Continuing with actual FPS reported by driver.",
                self,
                self.fps,
                actual_fps,
                success,
    ...
    def _validate_width_and_height(self) -> None:
        ...
        if abs(self.capture_width - actual_width) > 1:
            raise RuntimeError(
                f"{self} failed to set capture_width={self.capture_width} ({actual_width=}, {width_success=})."
            )

        actual_height = int(round(self.videocapture.get(cv2.CAP_PROP_FRAME_HEIGHT)))
        if abs(self.capture_height - actual_height) > 1:
            raise RuntimeError(
                f"{self} failed to set capture_height={self.capture_height} ({actual_height=}, {height_success=})."
            )

    def _apply_fourcc(self) -> None:
        """Apply preferred pixel format before validating other capture parameters."""
        if self.fourcc is None or not self.is_connected:
            return
        code = cv2.VideoWriter_fourcc(*self.fourcc)
        success = self.videocapture.set(cv2.CAP_PROP_FOURCC, code)
        actual = int(self.videocapture.get(cv2.CAP_PROP_FOURCC))
        if actual != code or not success:
            logger.warning(
                "%s failed to switch to fourcc=%s (success=%s, actual_code=%s)",
                self,
                self.fourcc,
                success,
                actual,
