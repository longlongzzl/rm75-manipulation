...
def _ensure_rs_available():
    if rs is None:
        raise ImportError(
            "pyrealsense2 is required to use RealSenseCamera but could not be imported. "
            "Install the appropriate RealSense SDK wheel for your platform."
        )
...
class RealSenseCamera(Camera):
    ...
    def latest_read(self) -> np.ndarray:
        with self.frame_lock:
            if self.latest_frame is None:
                raise RuntimeError("No frame available yet")
            return self.latest_frame.copy()

    def latest_read_with_id(self):
        with self.frame_lock:
            if self.latest_frame is None:
                return None, None
