...
class RealManArm(Robot):
    ...
    def __init__(self, config: RealManArmConfig):
        ...
        # Optional real-robot-only gripper target accumulator.  The ManiSkill
        # delta controller normally sends ``current_qpos + delta`` every
        # frame.  When the hardware is slower than the control loop, that
        # makes the next target stay close to the lagging feedback position.
        # Keeping a local absolute target lets the hardware chase the command
        # trajectory instead.  It is disabled by default and enabled by the
        # evaluator only for an explicit experiment.
        self._local_gripper_target_enabled: bool = False
        self._local_gripper_target: Optional[float] = None
        self._local_gripper_target_reset_pending: bool = False
        self._last_requested_gripper_target: Optional[float] = None
        self._local_gripper_direct_release_enabled: bool = True
        self._local_gripper_release_deadband: float = 0.002
        self._last_local_gripper_direct_release: bool = False
    ...
    def _get_udp_gripper_pos(self) -> Optional[float]:
        """Return the freshest UDP gripper position in the 0..0.91 convention."""
        if not self._udp_started:
            return None
        with self._udp_lock:
            raw = self._udp_gripper_raw
            last_ts = self._udp_last_ts
        if raw is None or time.time() - last_ts > self._udp_stale_s:
            return None
        return float(self._udp_gripper_to_obs(raw))

    def set_local_gripper_target(
        self,
        enabled: bool = True,
        initial: Optional[float] = None,
        direct_release: bool = True,
        release_deadband: float = 0.002,
    ) -> None:
        """Enable/disable real-only accumulation of absolute gripper targets.

        ``initial`` is expressed in the same 0..0.91 qpos convention as the
        environment.  If omitted, the latest UDP feedback is used when
        available.  Task resets are sent verbatim; after enabling this mode
        following a reset, subsequent policy targets are interpreted as
        ``feedback + delta`` and accumulated locally.

        When ``direct_release`` is enabled, a meaningful negative policy delta
        bypasses the local accumulator for that frame.  This keeps the local
        target useful for closing while allowing demonstrations to open the
        gripper without waiting for lagging feedback to catch up.
        """
        release_deadband = float(release_deadband)
        if not math.isfinite(release_deadband):
            raise ValueError(
                f"gripper release deadband is not finite: {release_deadband}"
            )
        self._local_gripper_target_enabled = bool(enabled)
        self._local_gripper_direct_release_enabled = bool(direct_release)
        self._local_gripper_release_deadband = max(0.0, release_deadband)
        if not self._local_gripper_target_enabled:
            self._local_gripper_target = None
            self._local_gripper_target_reset_pending = False
            self._last_requested_gripper_target = None
            self._last_local_gripper_direct_release = False
            return

        if initial is None:
            initial = self._get_udp_gripper_pos()
        if initial is not None and math.isfinite(float(initial)):
            self._local_gripper_target = max(0.0, min(0.91, float(initial)))
        else:
            self._local_gripper_target = None
        # This method is normally called after the evaluator has already
        # completed reset, so the next policy command should be accumulated
        # immediately.  ``reset()`` sets the pending flag itself when it needs
        # to send an absolute reset target.
        self._local_gripper_target_reset_pending = False
        self._last_requested_gripper_target = None
        self._last_local_gripper_direct_release = False
        print(
            "[Gripper local target] "
            f"enabled={self._local_gripper_target_enabled}; "
            f"initial={self._local_gripper_target}; "
            f"direct_release={self._local_gripper_direct_release_enabled}; "
            f"deadband={self._local_gripper_release_deadband:.4f}"
        )
    ...
    def _ensure_control_socket(self) -> None:
        if self.client is not None and self.client.fileno() >= 0:
            return

        ip = self.config.ip
        port_no = self.config.port
        ...
        try:
            ...
            raise
        self.client = client
        print("机械臂控制 socket 已连接", ip)

    def reset(
        self,
        arm_qpos_rad: Optional[Sequence[float]] = None,
        gripper_pos: Optional[float] = None,
    ) -> None:
        """Reset the real arm from a task qpos.

        ``arm_qpos_rad`` uses the same radian convention as the simulation.
        ``gripper_pos`` uses the real-agent convention: 0.0=open, 0.91=closed.
        Calling without arguments preserves the previous fixed reset behavior.
        """
        self._require_connected()
        assert self.arm is not None

        if arm_qpos_rad is None:
            arm_qpos_deg = [90.0, 0.0, 0.0, -90.0, 0.0, -90.0, 60.0]
        else:
            arm_qpos = torch.as_tensor(arm_qpos_rad, dtype=torch.float32).flatten()
            expected_dof = int(self._dof or 7)
            if arm_qpos.numel() != expected_dof:
                raise ValueError(
                    f"reset arm qpos has {arm_qpos.numel()} values, expected {expected_dof}"
                )
            if not torch.isfinite(arm_qpos).all():
                raise ValueError(f"reset arm qpos contains non-finite values: {arm_qpos}")
            arm_qpos_deg = torch.rad2deg(arm_qpos).tolist()

        if gripper_pos is not None:
            gripper_pos = float(gripper_pos)
            if not math.isfinite(gripper_pos):
                raise ValueError(f"reset gripper position is not finite: {gripper_pos}")
            if not -1e-6 <= gripper_pos <= 0.910001:
                raise ValueError(
                    f"reset gripper position {gripper_pos} is outside [0.0, 0.91]"
                )
            gripper_pos = max(0.0, min(0.91, gripper_pos))
            if self._local_gripper_target_enabled:
                # A task reset is an absolute command, not a policy delta.
                # Start a fresh local target for this episode.
                self._local_gripper_target = gripper_pos
                self._local_gripper_target_reset_pending = True
                self._last_requested_gripper_target = None
                self._last_local_gripper_direct_release = False
        else:
            # Preserve the old no-argument reset behavior.
            self.arm.rm_set_hand_follow_pos([0], False)
            if self._local_gripper_target_enabled:
                self._local_gripper_target = 0.0
                self._local_gripper_target_reset_pending = True
                self._last_requested_gripper_target = None
                self._last_local_gripper_direct_release = False

        self.arm.rm_movej(arm_qpos_deg, 100, 0, 0, 1)
        ...
        if gripper_pos is not None:
            self.send_action({"gripper.pos": gripper_pos})
    ...
    def send_action(self, action: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.config.enable_gripper and ("gripper.pos" in action):
            # The sim controller supplies an absolute qpos target in the
            # 0..0.91 convention, normally computed as feedback + one delta.
            requested_gripper_pos = max(
                0.0, min(0.91, float(action["gripper.pos"]))
            )
            command_gripper_pos = requested_gripper_pos

            local_direct_release = False
            if self._local_gripper_target_enabled:
                if self._local_gripper_target_reset_pending:
                    # Reset targets are absolute and must not be accumulated.
                    if self._local_gripper_target is None:
                        self._local_gripper_target = requested_gripper_pos
                    command_gripper_pos = self._local_gripper_target
                    self._local_gripper_target_reset_pending = False
                    self._last_local_gripper_direct_release = False
                else:
                    if self._local_gripper_target is None:
                        self._local_gripper_target = requested_gripper_pos
                    feedback_gripper_pos = self._get_udp_gripper_pos()
                    if feedback_gripper_pos is not None:
                        # Recover the delta that the sim controller intended,
                        # then apply it to our independent local target.  This
                        # deliberately does not use the lagging feedback as
                        # the new target base.
                        delta = requested_gripper_pos - feedback_gripper_pos
                    elif self._last_requested_gripper_target is not None:
                        # UDP can briefly be unavailable during startup.  Use
                        # the change in requested targets as a conservative
                        # fallback until feedback resumes.
                        delta = (
                            requested_gripper_pos
                            - self._last_requested_gripper_target
                        )
                    else:
                        delta = 0.0

                    # A jump larger than the controller per-step range is most
                    # likely an external/reset absolute command.  Follow it
                    # directly instead of making an unsafe accumulated jump.
                    if abs(delta) > 0.12:
                        self._local_gripper_target = requested_gripper_pos
                        command_gripper_pos = self._local_gripper_target
                        self._last_local_gripper_direct_release = False
                    elif (
                        self._local_gripper_direct_release_enabled
                        and delta < -self._local_gripper_release_deadband
                    ):
                        # Opening is sent directly for this frame so a stale
                        # local closing target cannot hold the gripper shut.
                        self._local_gripper_target = requested_gripper_pos
                        command_gripper_pos = requested_gripper_pos
                        local_direct_release = True
                        self._last_local_gripper_direct_release = True
                    else:
                        self._local_gripper_target = max(
                            0.0,
                            min(0.91, self._local_gripper_target + delta),
                        )
                        command_gripper_pos = self._local_gripper_target
                        self._last_local_gripper_direct_release = False

            self._last_requested_gripper_target = requested_gripper_pos
            # g = (0.91- float(action["gripper.pos"]))/0.91 #为0-1处理的函数 但是这里要放大一波 实际上传入的是0-0.91
            g = command_gripper_pos / 0.91  # 0..0.91 -> 0..1
            # delta_norm = g / 0.91
            ...
            if self._local_gripper_target_enabled:
                sent["gripper.local_target"] = float(command_gripper_pos)
                sent["gripper.local_direct_release"] = bool(local_direct_release)
