...
## 2026-09-13 current full M1-M10 delivery goal: precision run and actual offline FP

Overall PARTIAL_DELIVERY, not SOFTWARE_DELIVERY_READY. Current baseline 396bc2e18702c343c059993731b4f341b041c9ad plus recorded source.patch; all three pre-existing dirty changes retained. The user has explicitly authorized code, isolated tests/simulation and existing offline model inference, not real devices. All fourteen final software conditions and retained G0-G10 denominators remain required. Historical six-visual checkpoints do not supersede architecture section 4.

Kernel: 21 targeted passed / 0.41 s; latest full tests/ 1972 passed / 1 failed / 1 warning / 46.62 s. New endpoint event exposed an existing test assumption that every phase event contains ranked_ids. This failure is retained, not relabelled green. Previous 1973-pass full run belongs to the prior worktree.

Native wiring and actual simulation: formal pen context now asks the original backend for 0.0001 m position tolerance rather than 0.005 m. All collision/contact/holding/settle/goal criteria remain unchanged. Actual planned endpoint FK is reported explicitly as non-measured. Fixed-input formal run swm_pen_precision_XWbNVQ has eight endpoint errors below 1e-7 m and two private closure predictions without forbidden-contact rejection; six others reject under the unchanged protection. Full joint chain still fails with no feasible trajectory. This does not qualify grasp, place, release-retreat observation or recovery. All three primary/mirror/planner cleanup acknowledgements are true. The next investigation is lift/place joint feasibility after those two closure checks, not another collision exemption.

Model inference: actual existing FoundationPoseRefiner.global_register completed on the first local mustard0 RGB-D/mask frame with existing local refiner/scorer weights, seed 0, offline foundationpose310 environment. Output camera-frame pose and raw score 77.703125 are retained with all input/weight hashes. Inference completion is recorded separately; capture_time and RM75 world_extrinsic remain unknown rather than fabricated. No SWM checkpoint was admitted, no live camera used, and SAM3D remains NOT_RUN. This is actual model inference, not full G7/M4 completion or a claim of pose accuracy.

Hardware NOT_AUTHORIZED_NOT_RUN. Evidence: benchmarks/release/evidence/pen_precision_01/ and benchmarks/release/evidence/offline_fp_01/. Remaining software includes formal pen/recovery, full object set, Jimu two-base generated structures, actual rejected-candidate native continuation, two-push online posterior consumption, calibrated recorded-vision SWM admission, SAM3D metric registration, actual Agent recovery, constrained pull/rotate, unified console and final release regression. No software gap is relabelled hardware-only.
