...
## 2026-09-13 new grasp-place observation policy: first production wiring, native failure retained

Current requirements supersede the old six-visual-checkpoint acceptance: follow SWM_ARCHITECTURE_PROGRESS_AND_PLAN_20260913.md section 4 and CHATGPT_REVIEW_5E54D7D_OBSERVATION_POLICY_20260913.md. Observe before grasp (additional empty-hand pre-execution refresh is allowed), execute the paired original grasp/place, release and finish retreat, then independently observe. No object localization or continuous object tracking is required during holding. Robot/jaw feedback, original collision checks and faults remain mandatory. Predicted attachment is not a measurement or physical-identification sample.

Pulled origin successfully to f0bb5a8. Preserved the two earlier uncommitted files tests/three_scene/test_three_scene_pusht.py and tools/plan_swm_native_push.py; neither is included in this implementation commit. Added the paired observation policy to the actual pen context and both task/program consumers, preserving separately reported grasp/place primitives. Native phase planning now retains the selected joint placement candidate rather than discarding it. Full concatenated original stage auditing precedes execution; remaining lift/place can re-audit measured joint start/jaw geometry using the approved predicted attachment without reading a new object pose. The hidden measured-lift adapter is no longer installed in the new pen mode.

Added joint-only native readback and an explicit held-window object-capture guard. During the paired holding interval, execution feedback omits actor poses and object-settle reads are withheld; after release and completed retreat, original object settling and independent capture resume. Original simulation-specific pairwise-force closure and forbidden-contact protection remain, not a claim of equivalent real hardware sensing. SWM retains the original measured rows and their timestamps/sequence/source; a separate simulation_prediction execution_state marks the world unavailable to independent tasks until a final fresh checkpoint. A grasp receipt remains visual_verification_pending, never unconditionally skill_verified=true. Paired continuation is scoped to the trusted same-instance grasp/place context.

Three actual software probes exercise the new policy, not a stand-in native simulator: success, wrong final position and cancellation after closing. Their source throws if capture is called during holding; hidden lift re-audit completes without capture, measured object rows remain unchanged, release and retreat precede final capture, wrong placement requests replan and does not replay place, cancellation performs no later place or final capture. These probes are explicitly fixture evidence only.

SWM regression: 536 passed / 1 failed in 2.71 s. The remaining test constructs NativePrimaryExecutor via object.__new__ without paired_observation_mode, producing AttributeError before its expected missing-audit rejection. This first-pass compatibility issue was recorded and not patched again in this turn. Full tests/ not run on this patch.

Actual formal worker run: runtime_data/swm_paired_pen_worker_01, mandatory network isolation, original worker_65 profile/request, existing curobo2 Python, no real authorization, 600-second interrupt bound. Native initialization and new paired dispatch ran, then failed BEFORE planning with SceneInvalid: Request is not the active original compiled atom. The traceback connects atomic_groups -> paired policy -> plan_pair -> original CompiledNativeTask.build_task. Paired request iteration and the original active-atom lifetime are not yet aligned. No successful grasp/place or new-policy native observation sequence is claimed. Existing collision checks were not bypassed.

Kernel: partial regression with one failure. Native wiring: first actual policy installation, incomplete. Model inference NOT_RUN. Actual simulation: formal worker attempted and failed before planning. Hardware NOT_AUTHORIZED_NOT_RUN. Overall PARTIAL_DELIVERY. Evidence: benchmarks/release/evidence/paired_observation_01/ includes code/input hashes, complete software probe, SWM regression and actual native failure. Next: fix active-atom iteration and fixture compatibility, rerun formal worker and qualify final-outcome recovery, then address ordinary-versus-fatal PushT candidate rejection and the remaining parameter/FP/Jimu/Agent tasks. All remaining G0-G10 scope is retained.

## 2026-09-13 preserve original active-atom lifetime during paired execution

Previous turn was progress: new observation policy was wired into the formal worker, but eager list(requests) exhausted CompiledNativeTask.requests and cleared _active before planning. atomic_groups now consumes only the current grasp/place pair and advances the original generator after that pair finishes. Failed pairs do not consume the next atom. Single-skill iteration remains lazy, incomplete pairs reject before execution, and total consumed skills remain bounded at 64. The original compiler's active-atom check is unchanged, not bypassed.

Added four request-lifetime tests and parameterized the missing-postclosure-audit test for both legacy and paired observation modes, explicitly initializing its constructor-bypassing fixture field. Both modes must still reject the missing re-audit adapter. Full mandatory-isolated tests/: 1965 passed, 1 existing warning, 62.28 s. Tested worktree also contains the two pre-existing uncommitted files tests/three_scene/test_three_scene_pusht.py and tools/plan_swm_native_push.py; their hashes are recorded and they are not included in this commit. This is a tested-worktree result, not a clean-checkout release claim.

Actual formal worker runtime_data/swm_paired_pen_worker_02 used the identical worker_01 profile/request under network isolation, existing curobo2 Python and the same 600-second interrupt bound. The previous active-atom error is gone. Original relation search ran discrete and continuous-axis rounds within its unchanged eight-candidate motion budget; five closure candidates were explicitly rejected. Final result: RuntimeError: Native atomic phase has no feasible trajectory. The last retained closure error was NativeClosureRejected('Native closure prediction detected forbidden contact'). Do not generalize this last error as the sole cause for every candidate. Only worker_initialization_probe and before_grasp checkpoints occurred; the full paired movement and post-release observation have NOT passed.

Actual resource events report primary_closed=true, robot_mirror_closed=true and planner_pointer_released=true. Kernel: full tested-worktree regression green. Native wiring: lazy paired request lifetime now reached original planning. Actual simulation: formal primary initialization and private native closure candidate checks, task failed with no feasible trajectory. Model inference NOT_RUN; hardware NOT_AUTHORIZED_NOT_RUN. Overall PARTIAL_DELIVERY. No original contact allowlist, collision threshold, candidate budget or success tolerance was relaxed.

Evidence: benchmarks/release/evidence/paired_observation_02/ contains full test output, actual worker result/traceback, bounded checkpoint/relation/rejection/cleanup events, last closure summary and source/input hashes. Next: investigate exact candidate closure contacts and joint grasp/place feasibility under the same original assets/configuration; complete the formal paired execution/final observation and wrong-placement recovery before claiming support. Ordinary-versus-fatal PushT candidate rejection and remaining parameter/FP/Jimu/Agent requirements stay open. The current grasp-place contract remains architecture section 4, not the historical six visual checkpoints.

## 2026-09-13 concrete pen rejection diagnosis and ordinary candidate/fatal error separation

Previous turn was progress: active-atom lifetime fixed and formal pen worker reached original search, but found no feasible trajectory. This turn extracted all five real closure-candidate rejection reports from that run. Each rejected forbidden contact between a gripper support link and the workspace table: left/right support links, during gripper_close or closed_hold, with upward force components 2.1683210991692787, 0.5481336853981097, 4.218752031447411, 4.8756645219549055 and 3.850099157916736 N. These are actual retained private-PhysX predictions, not new primary actions. Positive reported point separations do not negate the nonzero native impulse-derived forces. No force epsilon, table collision exclusion or enlarged contact permission was introduced. The existing CachedClosurePriority already consumes bounded private closure predictions, so another duplicate priority layer was not added.

Implemented the separate requested multi-hypothesis failure boundary. Finite forbidden PushT collision evidence now has PushCollisionRejected; valid retreat-policy violations have RetreatContactRejected. Invalid identities, nonfinite/nonpositive depths and insufficient retreat evidence remain fatal validation errors. audit_future_push translates only explicit physical collisions, valid retreat violations and joint-limit infeasibility into CandidateInfeasible with stage/sample. _NativePushContext.evaluate catches only that dedicated type INSIDE the transaction guard, after the native audit restores its scene and the GPU context exits, returning feasible=false with bounded reason and parameter identity. It does not catch all SceneInvalid or unknown exceptions. Other candidates can continue, but a candidate failing any required hypothesis is ineligible under the unchanged minimax planner. Identity corruption, timeout, cancellation and unknown errors still close the entire factory.

Eight targeted software tests passed: one bad hypothesis eliminates A while B is selected; all rejected candidates yield bounded failure; a native-context rejection leaves the factory usable after context exit; identity, timeout, cancellation and nonfinite failures cleanly close; corrupt contact depth is not classified as ordinary infeasibility. These are software isolation/protocol cases, not an actual native A-reject/B-pass experiment.

Full mandatory-isolated tests/: 1970 passed / 3 failed / 1 existing warning, 45.31 s. Three existing motion-chain collision/self/neighbor fixtures omit penetration_m, and the new constructor currently raises KeyError rather than their expected RuntimeError/ValueError family. This is a newly exposed validation compatibility regression, retained without another edit in this turn. Missing depths must not be fabricated or treated as a normal candidate result; a subsequent fix must produce a controlled fatal validation error. The two prior dirty files remain outside this commit but are included in tested-worktree hashes.

Actual native positive regression: original GPU audit/scoring of candidate 0 nominal physical prediction ran again under mandatory network isolation and passed all 1880 samples, with original contact and retreat rules unchanged. No new CPU rollout or primary pen execution ran this turn. Evidence: benchmarks/release/evidence/candidate_rejection_01/ contains the five bounded pen rejection reports, exact source hashes, full/targeted tests and actual native positive audit. Kernel: targeted green, full regression NOT_GREEN. Native wiring: typed candidate rejection added; actual native negative-search qualification remains open. Model inference NOT_RUN; hardware NOT_AUTHORIZED_NOT_RUN. Overall PARTIAL_DELIVERY. Next: repair the controlled missing-depth error contract, qualify rejected-candidate continuation natively, and continue the pen joint-grasp search without relaxing its support/table protection or the current no-held-object-observation policy.

## 2026-09-13 controlled depth validation repair and linear-planning investigation

The two previously retained changes were committed as 396bc2e: deterministic PushT freshness tests and the explicit native-audit cancellation probe. That commit did not run tests or the probe and was not pushed in that turn.

PushCollisionRejected now converts a missing or unparseable penetration_m into an explicit fatal ValueError, preserving the original exception as its cause. Finite positive depth validation remains mandatory. No default depth, collision bypass or ordinary CandidateInfeasible result is introduced for malformed evidence. This repair has not been tested; the historical 1970 passed / 3 failed result remains the last full regression evidence, not a result for the repaired worktree.

Text inspection of the original PickPlaceCoordinator._plan_linear_stage shows that it forwards the candidate to plan_linear_candidates and returns result.best without a separate endpoint refinement in that method. The curobo2 backend declares a default position_tolerance of 0.005 m. These facts do not establish the effective linear-stage tolerance in the formal pen run or prove the cause of support/table contact. A cached-IK versus solved-endpoint discrepancy remains a hypothesis requiring endpoint FK and effective solver-setting evidence. No planner tolerance, endpoint, contact rule or model geometry was changed to make a candidate pass.

Kernel: depth error-contract repair implemented, regression NOT_RUN on this repair. Native wiring: paired policy previously installed, formal grasp-place completion still unqualified. Model inference: NOT_RUN. Actual simulation: NOT_RUN in these follow-up turns; prior worker_02 planning failure remains authoritative. Hardware: NOT_AUTHORIZED_NOT_RUN; no device operation. Overall PARTIAL_DELIVERY.

Next: with explicit execution approval, run the isolated candidate-rejection and motion-chain regressions; establish the actual linear endpoint residual and closure initialization difference before selecting a planning correction; then qualify the complete before-grasp / release-and-retreat / final-observation sequence. Native rejected-candidate continuation, online physical-parameter consumption, FoundationPose/SAM3D, other tasks and the full G0-G10 requirements remain open. No old six-visual-checkpoint acceptance is reinstated.

## 2026-09-13 explicitly authorized isolated regression and formal pen rerun

User explicitly authorized network-isolated regression tests and native pen simulation, excluding all real devices. No real authorization flag was supplied. The copied request remains task=pickplace, mode=sim, object_name=bi; hardware_reviewed remains false. Existing assets, original planner/executor and collision policies were retained without a threshold change.

Kernel: targeted candidate-rejection and motion-chain regression completed with 30 passed in 1.50 s. Full formal tests/ regression, run serially after the native worker terminated, completed with 1973 passed and one trimesh deprecation warning in 48.09 s. Both used PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 and python3 tools/run_network_isolated.py -- python3 -m pytest with explicit test paths. The controlled missing-depth ValueError repair is now covered by the previously failing motion-chain cases. This is a current-worktree result, not a clean committed-release qualification.

Native wiring / actual simulation: original workcell.worker ran under the mandatory network wrapper and a 600-second timeout using the unchanged prior profile/request copied into runtime_data/swm_paired_pen_authorized_AnMQWe/. It terminated with exit code 1 and RuntimeError: Native atomic phase has no feasible trajectory. The actual traceback reaches paired_grasp_place.run -> PickPlaceNativePhases.plan_pair -> plan. Original private closure predictions rejected candidates; retained final events report primary_unchanged=true and private_robot_closed=true. Main executor drive-command release reports zero arm and gripper writes. No complete primary grasp/place or final post-release observation is claimed. Cleanup reports primary_closed=true, robot_mirror_closed=true and planner_pointer_released=true.

Model inference: NOT_RUN. Hardware: NOT_AUTHORIZED_NOT_RUN; no real device connected or operated. Overall PARTIAL_DELIVERY. Software regression success does not qualify formal grasp-place execution. The next implementation investigation remains actual planned-endpoint versus cached-IK closure behavior, without assuming the default backend tolerance is the cause and without relaxing collision checks. The new observation policy and all remaining parameter-loop / other-task / G0-G10 requirements remain intact.

Evidence: runtime_data/swm_paired_pen_authorized_AnMQWe/{request.json,profile.json,console.log,result.json,traceback.txt,events.jsonl,full_tests.log,swm_native/closure_candidates/}. The native launch used /home/zhangzhao/anaconda3/envs/curobo2/bin/python -m rm75_app.workcell.worker with --run-dir, --profile and --app-root only, wrapped by python3 tools/run_network_isolated.py -- timeout --signal=INT --kill-after=30s 600s. No real-device command was used.

## 2026-09-13 full delivery goal continuation: tighter original solve and first actual offline FP

Preserved baseline 396bc2e and all existing dirty changes. Formal pen production backend now requests 0.0001 m position convergence; collision, jaw, contact, settle and goal thresholds are unchanged. Planned endpoint FK events explicitly carry source=planned_endpoint_fk and measured=false. Actual original worker run runtime_data/swm_pen_precision_XWbNVQ: eight endpoints below 1e-7 m position error; two closure predictions not rejected, six rejected. Full joint grasp/place chain remains infeasible. Native resources were released. Do not equate closure non-rejection with skill success or final observation.

Kernel: 21 targeted passed, latest full regression 1972 passed / 1 failed. Failure is test_native_relation_continuation.py reading ranked_ids from the new endpoint event; no second code patch or hidden passing rerun replaced this failure. Native wiring PARTIAL; actual pen task FAILED; no completed release-retreat checkpoint. Next isolate lift/place feasibility after the two non-rejected closure candidates.

Actual model inference now ran through existing FoundationPoseRefiner on the local mustard0 first RGB-D frame and mask, using existing hashed weights and original mesh, seed 0. It returned source=foundationpose_spherical and raw quality score 77.703125. Capture provenance and RM75 world transform are not established for this example: no measured SWM update and no M4/G7 completion claim. Inference completion time is recorded only as inference time. SAM3D not run. Hardware NOT_AUTHORIZED_NOT_RUN.

The rolling plan, unique machine-readable acceptance and final summary now carry the user's full M1-M10 scope and retained G0-G10 requirements. Evidence is in benchmarks/release/evidence/pen_precision_01/ and offline_fp_01/. Overall PARTIAL_DELIVERY; remaining independent work continues under already granted isolated software permission.
