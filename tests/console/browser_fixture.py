#!/usr/bin/env python3
"""Real browser + production WSGI/ConsoleAPI, explicit fake runtime/model.

This process listens only on loopback. It never imports the real IterationAPI,
WorkcellService, worker, SDK or solver. All browser traffic is restricted to that
single test origin. This is NOT native/LLM/physics integration qualification.
"""
...
def main():
    ...
    output=Path(sys.argv[1] if len(sys.argv)>1 else '/mnt/data/rm75_console_browser')
    output.mkdir(parents=True,exist_ok=True)
    report={'runtime':'UI_FIXTURE_ONLY','network_model_calls':0,'robot_imported':False,'hardware_connected':False,
            'native_worker_run':False,'physics_run':False,'checks':[],'page_errors':[]}
    ...
    server=make_server('127.0.0.1',0,app,server_class=Server,handler_class=Quiet)
    ...
    try:
        with sync_playwright() as pw:
            ...
            page.goto(origin+'/workcell/console/',wait_until='networkidle')
            ...
            page.reload(wait_until='networkidle');expect(page.locator('#restore-note')).to_contain_text('恢复')
            assert page.locator('#object-grid input:checked').count()==7 and len(service.submissions)==1
            check('preview + page reload restores draft and never repeats submission')
            ...
            assert len(app.iteration.calls)==1
            check('slow generation >135s logical elapsed + reload resumes without another call')
            ...
            context.set_offline(True);page.click('#refresh')
            expect(page.locator('#connection')).to_contain_text('连接中断',timeout=15000)
            previous=len(service.submissions);context.set_offline(False);page.click('#refresh')
            ...
            report.update(status='PASS',http_requests=len(requests),task_submissions=len(service.submissions),model_substitute_calls=len(app.iteration.calls))
            browser.close()
    except Exception as exc:
        report.update(status='FAILED',error=str(exc))
        raise
    finally:
        server.shutdown();atomic_json(output/'report.json',report)
    return 0
